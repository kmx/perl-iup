...
means of the L<Destroy|[%m.elem%]/"Destroy()"> function. To destroy an image, it cannot be
...
that points to names registered with L<SetName|[%m.elem%]/"SetName()">. You can also use
XXX-FIX_ME to shortcut the set of an image as an
...
L<IUP::Toggle|IUP::Toggle>, L<Destroy|[%m.elem%]/"Destroy()">.
