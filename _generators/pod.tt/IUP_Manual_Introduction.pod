...
[% m.intro %] - Basic introduction to IUP concept

=head1 IUP MANUAL

=over

=item * [%m.intro%] E<nbsp>E<nbsp>E<nbsp>E<nbsp>E<nbsp> B<E<lt>E<lt>E<lt> this document>
...
=item * L<SetAttribute|[%m.elem%]/"SetAttribute()"> stores only a pointer to the string and does
not duplicate it.

=item * L<StoreAtribute|xxx> duplicates the string, allowing you to use
it for other purposes.

=back

With L<SetAttribute|[%m.elem%]/"SetAttribute()"> you can also store application pointers that
...
pointer inside the callback through function L<GetAttribute|[%m.elem%]/"GetAttribute()">.
...
L<SetAttribute|[%m.elem%]/"SetAttribute()> / L<GetAttribute|[%m.elem%]/"GetAttribute()> logic can work, even if the control class store it
...
must employ the L<SetAttribute|[%m.elem%]/"SetAttribute()"> function, linking the action to a
name (passed as a string). From this point on, this name will refer to
a callback. By means of function L<SetFunction|[%m.elem%]/"SetFunction()">, the user connects
...
using L<SetFunction|[%m.elem%]/"SetFunction()">. For example:
...
L<SetCallback|[%m.elem%]/"SetCallback()">. For example:
...
name in the LED, and replace L<SetFunction|[%m.elem%]/"SetFunction()"> by L<SetCallback|[%m.elem%]/"SetCallback()">.
...
is not called when the programmer sets a value via L<SetAttribute|[%m.elem%]/"SetAttribute()">.
...
The L<LoopStep|IUP/"LoopStep()"> and the L<Flush|[%m.elem%]/"Flush()"> functions force the processing
