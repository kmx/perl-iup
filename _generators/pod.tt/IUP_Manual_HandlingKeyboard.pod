...
[% m.keys %] - Handling keyboard in IUP applications
...
=item * [%m.keys%] E<nbsp>E<nbsp>E<nbsp>E<nbsp>E<nbsp> B<E<lt>E<lt>E<lt> this document>
...
L<GetFocus|IUP/"GetFocus()"> and L<SetFocus|[%m.elem%]/"SetFocus()">. When the focus is changed the
...
is when the focus is at an L<IUP::Multiline|IUP::Multiline> control, to change focus the
...
functions L<NextField|[%m.elem%]/"NextField()"> and L<PreviousField|[%m.elem%]/"PreviousField()">. To remove the Tab
...
=item * C<IUP-E<gt>isprint($key)> function informs if a key can be directly used as
a printable character

=item * C<IUP-E<gt>isXkey($key)> function informs if a given
key is an extended code

=item * C<IUP-E<gt>isShiftXkey($key)> function informs if a
given key is an extended code using the Shift modifier

=item * C<IUP-E<gt>isCtrlXkey($key)> function for the Ctrl modifier

=item * C<IUP-E<gt>isAltXkey($key)> function for the Alt modifier

=item * C<IUP-E<gt>isSysXkey($key)> function for the Sys modifier xxxTODO globally replace 'macro' > 'function'
 ...
 *		K_asterisk
 ...
 Context Menu	K_Menu
 XXX		K_acute    xxxencoding broken
 XXX		K_ccedilla xxxencoding broken
 F1		K_F1
 F2		K_F2
 F3		K_F3
 F4		K_F4
