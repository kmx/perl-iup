=for comment based on iup-3.9 (attributes refreshed to iup-3.32) - http://www.tecgraf.puc-rio.br/iup/en/ctrl/iupmatrixlist.html
...
The text color when an item get the focus. If not defined uses the
B<ITEMFGCOLORI(id)>. (since iup-3.29)
...
moment the event is generated. The same macros used for L<BUTTON_CB|[%m.cb%]/BUTTON_CB> can be used for this status.
...
Action generated when any mouse button is pressed or released over the list.
...
B<$pressed:> equal to 1 if the button was pressed or to 0 if the button was released.

B<$x, $y:> position in the canvas where the event has occurred, in pixels.

B<$status:> status of the mouse buttons and some keyboard keys at the
moment the event is generated, given as a string.

=back

[% h.examples %]

[% IF examples.iupmatrixlist %]
The element B<IUP::MatrixList> is used in the following sample scripts:

=over
[%FOREACH e IN examples.iupmatrixlist %]
...
The original doc: L<iupmatrixlist.html|[%url.iuporigdoc%]ctrl/iupmatrixlist.html>
[%END%]
