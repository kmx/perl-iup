...
=item B<ATTRIBCHANGED_CB>

Action generated when an attribute is interactively changed in the
dialog. The affected element is available via the dialog's B<ELEM>
attribute.

[%h.at_handler%]

 sub attribchanged_cb_handler {
   my ($self, $name) = @_;
   #...
 }

B<$self:> reference to the element (L<IUP::LayoutDialog|IUP::LayoutDialog>) that activated the event.

B<$name:> name of the changed attribute.

=item B<LAYOUTCHANGED_CB>

Action generated when the layout being edited is changed.

[%h.at_handler%]

 sub layoutchanged_cb_handler {
   my ($self, $elem) = @_;
   #...
 }
...
B<$elem:> the element whose layout changed.
...
The original doc: L<iuplayoutdialog.html|[%url.iuporigdoc%]dlg/iuplayoutdialog.html>
[%END%]
