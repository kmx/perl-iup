...
IUP::Popup function only.

[% h.usage %]

[% h.create %]

 Ihandle* IUP::FileDlg(void); [in C]
...
=item * B<L<PARENTDIALOG|[%m.at%]/PARENTDIALOG>>
...
xxxTODO (check original)??? Read only attributes that are valid inside the FILE_CB callback when
status="PAINT":
...
=item * B<L<TITLE|[%m.at%]/TITLE>>

Dialog's title.

=item * B<VALUE> (read-only)

Name of the selected file(s), or NULL if no file
...
=item * B<file_name> - name of the file selected.

=item * B<status> - describes the action. Can be:
...
To show the dialog, use function L<IUP::Popup|IUP::Popup>. In
Lua, use the popup function.

The dialog is mapped only inside L<IUP::Popup|IUP::Popup>, L<IUP::Map|IUP::Map> does nothing.

The L< IUP::GetFile|iupgetfile.html> function simply creates and popup a
IUP::FileDlg.
...
[% h.examples %]
 ...
 Ihandle *dlg = IUP::FileDlg(); 
 
 IUP::SetAttribute(dlg, "DIALOGTYPE", "OPEN");
 IUP::SetAttribute(dlg, "TITLE", "IUP::FileDlg Test");
 IUP::SetAttributes(dlg, "FILTER = \"*.bmp\", FILTERINFO = \"Bitmap Files\"");
