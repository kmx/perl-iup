=for comment based on iup-3.2 (attributes refreshed to iup-3.32) - http://www.tecgraf.puc-rio.br/iup/en/elem/iupcanvas.html
...
only during the ACTION callback and only when using GTK version 3.
(since iup-3.7)
...
Enable or disable the drop of files. Default: NO, but if DROPFILES_CB
is defined when the element is mapped then it will be automatically
enabled. (in older versions this attribute was named DRAGDROP)
...
Returns the name of the draw driver in use by the IUP::Draw API. Can
be: X11 (Motif), GDK (GTK), Cairo (GTK), D2D (Windows), GDI+ (Windows)
or GDI (Windows). (since iup-3.25)

=item B<DRAWUSEGDI>

I<[Windows Only] (non inheritable)>

Force the use of the old GDI driver, instead of the new DirectD2
driver. Used in the L<IUP::Gauge|IUP::Gauge>, L<IUP::Matrix|IUP::Matrix>
and Flat Scrollbars for better performance and backward compatibility.
(since iup-3.26)
...
Disable the gesture support for touch interfaces. Accepts only the NO
value. (since iup-3.31)

=item B<WHEELDROPFOCUS>

I<(non inheritable)>

When the wheel is used the focus control receives a SHOWDROPDOWN=No.
(since iup-3.28)
...
=item L<KEYPRESS_CB|[%m.cb%]/KEYPRESS_CB>
...
=item L<SCROLL_CB|[%m.cb%]/SCROLL_CB>
...
=item L<WHEEL_CB|[%m.cb%]/WHEEL_CB>

Action generated when the mouse wheel is rotated.

=item L<WOM_CB|[%m.cb%]/WOM_CB>
...
this, return IGNORE in the L<K_ANY|[%m.cb%]/K_ANY> callback.

=item * The mouse cursor position can be programmatically controlled using the
global attribute L<CURSORPOS|[%m.at%]/CURSORPOS>.
...
transparent white). Use C<cdSetForeground()> to avoid the conflict. See
...
transparent white). Use C<cdSetBackground()> to avoid the conflict. See
also L<Color Coding|color.html>.
...
But notice that the indices are different of the C<cdVectorTextTransform()>.

Functions that retrieve images from the canvas are not affected by the
transformation matrix, such as C<GetImage()> and C<ScrollArea()>.
...
=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/wc-dc.gif"></p>
...
=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/marks.gif"></p>
...
=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/arc.gif"></p>
...
the C<cdLineStyleDashes()> function must be called before to initialize
...
=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/lines.gif"></p>
...
=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/linejoin.gif"></p>
...
=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/linecap.gif"></p>
...
C<cdCanvasPathSet()> to configure the action between sequences of
C<cdCanvasVertex()>.
...
=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/polygon.gif"></p>

Bezier Lines:

=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/bezier.gif"></p>
...
Configures the action between sequences of C<cdCanvasVertex()>. B<action>
can be:

=over

=item * B<CD_PATH_NEW> - creates a new empty path. Useful if more than
one path is configured. C<cdCanvasBegin()>(CD_PATH) already creates a new
path.

=item * B<CD_PATH_MOVETO> - moves the current position to the given
coordinates. Must be followed by 1 call to C<cdCanvasVertex()>, 
...
1 call to C<cdCanvasVertex()>, C<cdfCanvasVertex()>, or B<wdCanvasVertex>.
...
by 3 calls to C<cdCanvasVertex()>, B<cdfCanvasVertex>, or
...
C<cdCanvasVertex()>, B<cdfCanvasVertex>, or B<wdCanvasVertex>. Must be
...
function behaves like its equivalent C<cdRect()>.
...
=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/sector.gif"></p>
...
When the interior style B<CD_HOLLOW> is defined, the function behaves
like its equivalent C<cdArc()>, plus a line connecting the arc start and
end points.

Chord Parameters:

=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/chord.gif"></p>
...
=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/opacity.gif"></p>
...
=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/fillmode.gif"></p>
...
C<cdSector()> behave as their equivalent C<cdRect()> and C<cdArc()>,
...
=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/hatch.gif"></p>
...
relative to this point using the C<cdTextAlignment()> function.
...
=for html <p>
  <table border="1">
    <tbody align="center">
      ...
      <tr>
        <td><img src="[%url.ghwebroot%]/imgcd-5.4.1/fonts.gif"></td>
        <td><img src="[%url.ghwebroot%]/imgcd-5.4.1/font_style.gif"></td>
      </tr>
    </tbody>
  </table>
</p>
...
=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/align.gif"></p>
...
=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/font_dim.gif"></p>
...
=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/vector_text.gif"></p>
...
=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/vectorfont_default.png"></p>
...
=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/vectorfont_simplex2.png"></p>
...
C<wdPutImage()> are the only methods for manipulating Server Images.
...
via L<cdPutImage|/"cdPutImage()"> or C<wdPutImage()>). 
...
See more info about L<cdBegin|/"cdBegin()"> and C<cdBegin()>.
...
=for html <p><img src="[%url.ghwebroot%]/imgcd-5.4.1/regions.gif"></p>
...
=over
[%FOREACH e IN examples.iupcanvas %]
...
The original doc: L<iupcanvas.html|[%url.iuporigdoc%]elem/iupcanvas.html>
[%END-%]
