...
[% m.at %] - Attribute concept, using attributes, common + global attributes

=head1 INTRODUCTION

xxxTODO link to attribute concept in IUP::Manual::BasicConcept

xxxTODO this is just a refference guide for common and/or frequently used attributes
...
L<ACTIVE|/ACTIVE>, L<BGCOLOR|/BGCOLOR>, L<FGCOLOR|/FGCOLOR>, L<FONT|/FONT>, L<FONTSTYLE|/FONTSTYLE>,
L<FONTSIZE|/FONTSIZE>, L<FONTFACE|/FONTFACE>, L<CHARSIZE|/CHARSIZE>, L<FOUNDRY|/FOUNDRY>, L<VISIBLE|/VISIBLE>,
L<CLIENTSIZE|/CLIENTSIZE>, L<CLIENTOFFSET|/CLIENTOFFSET>, L<EXPAND|/EXPAND>, L<MAXSIZE|/MAXSIZE>, L<MINSIZE|/MINSIZE>,
L<RASTERSIZE|/RASTERSIZE>, L<SIZE|/SIZE>, L<FLOATING|/FLOATING>, L<POSITION|/POSITION>, L<X|/X>, L<Y|/Y>,
L<NAME|/NAME>, L<TIP|/TIP>, L<TIPBALLON|/TIPBALLON>, L<TIPBALLONTITLE|/TIPBALLONTITLE>, L<TIPBALLONTITLEICON|/TIPBALLONTITLEICON>,
L<TIPBGCOLOR|/TIPBGCOLOR>, L</TIPDELAY >, L<TIPFGCOLOR|/TIPFGCOLOR>, L<TIPFONT|/TIPFONT>, L<TIPICON|/TIPICON>,
L<TIPMARKUP|/TIPMARKUP>, L<TIPRECT|/TIPRECT>, L<TIPVISIBLE|/TIPVISIBLE>, L<TITLE|/TITLE>, L<VALUE|/VALUE>,
L<WID|/WID>, L<ZORDER|/ZORDER>
...
L<MASKINT|/MASKINT>, L<MASKFLOAT|/MASKFLOAT>, L<PARENTDIALOG|/PARENTDIALOG>, L<POSX|/POSX>, L<POSY|/POSY>,
L<SCROLLBAR|/SCROLLBAR>, L<SHRINK|/SHRINK>, L<XMAX|/XMAX>, L<XMIN|/XMIN>, L<YMAX|/YMAX>, L<YMIN|/YMIN>
...
L<LANGUAGE|/LANGUAGE>, L<VERSION|/VERSION>, L<COPYRIGHT|/COPYRIGHT>, L<DRIVER|/DRIVER>

=item * B<L<Global Attributes - System Control|/Global Attributes - System Control>>

L<LOCKLOOP|/LOCKLOOP>, L<CURSORPOS|/CURSORPOS>, L<MOUSEBUTTON|/MOUSEBUTTON>, L<SHIFTKEY|/SHIFTKEY>, L<CONTROLKEY|/CONTROLKEY>,
L<MODKEYSTATE|/MODKEYSTATE>, L<KEYPRESS|/KEYPRESS>, L<KEYRELEASE|/KEYRELEASE>, L<KEY|/KEY>, L<AUTOREPEAT|/AUTOREPEAT>, L<UTF8AUTOCONVERT|/UTF8AUTOCONVERT>
L<SINGLEINSTANCE|/SINGLEINSTANCE>

=item * B<L<Global Attributes - System Information|/Global Attributes - System Information>>

L<SYSTEMLANGUAGE|/SYSTEMLANGUAGE>, L<SYSTEM|/SYSTEM>, L<SYSTEMVERSION|/SYSTEMVERSION>, L<GTKVERSION|/GTKVERSION>, L<GTKDEVVERSION|/GTKDEVVERSION>,
L<MOTIFVERSION|/MOTIFVERSION>, L<MOTIFNUMBER|/MOTIFNUMBER>, L<COMPUTERNAME|/COMPUTERNAME>, L<USERNAME|/USERNAME>, L<XSERVERVENDOR|/XSERVERVENDOR>,
L<XVENDORRELEASE|/XVENDORRELEASE>

=item * B<L<Global Attributes - Screen Information|/Global Attributes - Screen Information>>

L<FULLSIZE|/FULLSIZE>, L<SCREENSIZE|/SCREENSIZE>, L<SCREENDEPTH|/SCREENDEPTH>, L<TRUECOLORCANVAS|/TRUECOLORCANVAS>, 
L<VIRTUALSCREEN|/VIRTUALSCREEN>, L<MONITORSINFO|/MONITORSINFO>

=item * B<L<Global Attributes - System Data|/Global Attributes - System Data>>

L<HINSTANCE|/HINSTANCE>, L</DLL_HINSTANCE>, L<APPSHELL|/APPSHELL>, L<XDISPLAY|/XDISPLAY>, L<XSCREEN|/XSCREEN>

=item * B<L<Global Attributes - Default Attributes|/Global Attributes - Default Attributes>>

L<DLGBGCOLOR|/DLGBGCOLOR> , L<DLGFGCOLOR|/DLGFGCOLOR>, L<MENUBGCOLOR|/MENUBGCOLOR>, L<MENUFGCOLOR|/MENUFGCOLOR>,
L<TXTBGCOLOR|/TXTBGCOLOR>, L<TXTFGCOLOR|/TXTFGCOLOR>, L<DEFAULTFONT|/DEFAULTFONT>, L<DEFAULTFONTSIZE|/DEFAULTFONTSIZE>
...
ElementE<rsquo>s foreground color. Usually it is the color of the
associated text.
...
When FONT is changed and L<SIZE|/SIZE> is set, then
L<RASTERSIZE|/RASTERSIZE> is also updated.
...
xxx=head4 Font - Auxiliary Attributes
...
=item B<FOUNDRY> (non inheritable) [Motif Only]
...
xxxx=head4 ISO8859-1 and Windows-1252 Displayable Characters
...
the table bellow with thick borders.

xxx TODO xxx table

Adapted from L<http:E<47>E<47>en.wikipedia.orgE<47>wikiE<47>Windows-1252|http://en.wikipedia.org/wiki/Windows-1252>.

[% h.at_see %] L<IUP::MapFont|IUP::MapFont>, L<IUP::UnMapFont|IUP::UnMapFont>. xxxTODOXX(maybe remove)
...
=item B<CLIENTSIZE> (read-only**, non inheritable)
...
Guide|../layout_guide.html>. Useful for L<IUP::Frame|IUP::Frame>, L<IUP::Tabs|IUP::Tabs> and
...
[% h.at_value %] "(width)x(height)", where (width) and (height) are integer
...
=item B<CLIENTOFFSET> (read-only, non inheritable)
...
[% h.at_value %] "I<dx>xI<dy>", where I<dx> and I<dy> are integer values corresponding
...
=item B<MAXSIZE> (non inheritable)

Specifies the element maximum size in pixels during the layout process.

See the L<Layout Guide|../layout_guide.html> for more details on sizes.

[% h.at_value %] "(width)x(height)", where (width) and (height) are integer values
...
=item B<MINSIZE> (non inheritable)
...
=item B<RASTERSIZE> (non inheritable)
...
in L<IUP::GetAttribute|IUP::GetAttribute> will still be the B<Current> size.

The element is NOT immediately repositioned. Call L<IUP::Refresh|IUP::Refresh> to
update the dialog layout.

L<IUP::Map|IUP::Map> also updates the dialog layout even if it is already mapped,
so calling it or calling L<IUP::Show|IUP::Show>, L<IUP::ShowXY|IUP::ShowXY> or L<IUP::Popup|IUP::Popup>
...
=item B<FLOATING> (non inheritable)
...
[% h.at_value %] "YES", "IGNORE" (since 3.3) or "NO".

[% h.at_default %] "NO"

[% h.at_affects %] All elements, except menus.

[% h.at_see %] L<IUP::Zbox|IUP::Zbox>, L<IUP::VBox|IUP::VBox>,
L<IUP::HBox|IUP::HBox>

=item B<POSITION> (read-only) (non inheritable)
...
[% h.at_value %] "E<lt>xE<gt>,E<lt>yE<gt>", where E<lt>xE<gt> and E<lt>yE<gt>
are integer values corresponding to the horizontal and vertical position, 
...
=item B<NAME> (non inheritable)

Name of the control inside the dialog. Not releated to
L<IUP::SetHandle|IUP::SetHandle>.

[% h.at_value %] Text.

[% h.at_notes %]

The NAME value will be used by
L<IUP::GetDialogChild|IUP::GetDialogChild> to find a child
inside a dialog.

[% h.at_affects %] All controls.

[% h.at_see %] L<IUP::GetDialogChild|IUP::GetDialogChild>

=item B<TIP> (non inheritable)
...
xxx=head4 Additional Tip Attributes (Windows and Motif Only)

These attributes affect the TIP display.

=over

=item B<TIPBALLON> [Windows Only]
...
=item B<TIPBGCOLOR> [Windows and Motif Only]

The tip background color.

[% h.at_default %] "255 255 225" (Light Yellow)

=item B<TIPDELAY> [Windows and Motif Only]

Time the tip will remain visible.

[% h.at_default %] "5000"

=item B<TIPFGCOLOR> [Windows and Motif Only]

The tip text color.

[% h.at_default %] "0 0 0" (Black)

=item B<TIPFONT> [Windows and Motif Only]
...
=item B<TIPRECT> (non inheritable)
...
must be defined by the L<IUP::SetHandle|IUP::SetHandle>
...
=item B<WID> (read-only) (non inheritable)
...
=item B<ZORDER> (write-only, non inheritable)
...
=head1 Global Attributes - General
...
Can also be set by L<IUP::SetLanguage|IUP::SetLanguage>.

=item B<VERSION> (read-only)
...
=head1 Global Attributes - System Control

=over

=item B<LOCKLOOP>

When the last visible dialog is closed the L<IUP::ExitLoop|IUP::ExitLoop> function is
...
=item B<UTF8AUTOCONVERT> [GTK Only]
...
=item B<SINGLEINSTANCE> [Windows Only]
...
of the second instance. When consulted return NULL if inside the second
instance. So usually in the application initialization after
L<IUP::Open|IUP::Open>, set SINGLEINSTANCE and then consult its value, if NULL
abort the second instance by calling B< IUP::Close> and returning from
I<main>.

=back

=head1 Global Attributes - System Information

=over

=item B<SYSTEMLANGUAGE> (read-only)

Return respectively a text with a description of the system language.

=item B<SYSTEM> (read-only)
...
=item B<SYSTEMVERSION> (read-only)
...
=item B<GTKVERSION> (read-only) [GTK Only]
...
=item B<SCREENDEPTH> (read-only)

Returns the screen depth in bits per pixel.

=item B<TRUECOLORCANVAS> (read-only)
...
=item B<VIRTUALSCREEN> (read-only)  [Win32 and GTK Only]
...
=item B<MONITORSINFO> (read-only) [Win32 and GTK Only]
...
=head1 Global Attributes - System Data

=over

=item B<HINSTANCE> (read-only) [Win32 Only]

Returns a handle (HINSTANCE) that identifies the application in the
native system.

=item B<DLL_HINSTANCE> [Win32 Only]

Changes and returns a handle (HINSTANCE) that identifies the DLL where
resources are stored.

=item B<APPSHELL> (read-only) [Motif Only]

Returns the shell Widget created by XtOpenApplication.

=item B<XDISPLAY> (read-only) [GTK and Motif Only]

Returns the X-Windows Display.

=item B<XSCREEN> (read-only) [GTK and Motif Only]

Returns the X-Windows Screen.

=back

=head1 Global Attributes - Default Attributes
...
=head1 Frequently used Attributes
...
find an IUP image with the same name. Use L<IUP::SetHandle|IUP::SetHandle> to define a
...
through function L<IUP::Destroy|IUP::Destroy>. Attention: the
...
Use L<IUP::SetHandle|IUP::SetHandle> or
...
=item B<MASKCASEI> (non inheritable)
...
=item B<MASKINT> (non inheritable, write only)
...
=item B<MASKFLOAT> (non inheritable, write only)
...
redraw call L<IUP::Update|IUP::Update> in L<SCROLL_CB|[%m.cb%]SCROLL_CB>.
