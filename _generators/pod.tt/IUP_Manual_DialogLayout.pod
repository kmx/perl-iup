...
[% m.dlg %] - Dialog layout composition
...
=item * [%m.dlg%] E<nbsp>E<nbsp>E<nbsp>E<nbsp>E<nbsp> B<E<lt>E<lt>E<lt> this document>
...
native system and every time L<Map|[%m.elem%]/"Map()"> is called, even if the element
is already mapped.
...
L<Refresh|[%m.elem%]/"Refresh()"> is called.
...
L<Create|[%m.elem%]/"Create()"> to create branches or nodes. To
destroy a node or branch call L<Destroy|[%m.elem%]/"Destroy()">.

Some of the constructors already append children to its branch, but you
can add other children using L<Append|[%m.elem%]/"Append()"> or
L<Insert|[%m.elem%]/"Insert()">. To remove from the tree call
L<Detach|[%m.elem%]/"Detach()">.

For the element to be visible L<Map|[%m.elem%]/"Map()"> must be called
so it can be associated with a native control. L<Show|[%m.elem%]/"Show()">, L<ShowXY|[%m.elem%]/"ShowXY()"> or L<Popup|[%m.elem%]/"Popup()"> will automatically call L<Map|[%m.elem%]/"Map()"> 
before showing a dialog. To remove this association call L<Unmap|[%m.elem%]/"Unmap()">.
...
when the constructor receives the children as a parameter L<Append|[%m.elem%]/"Append()">
...
hierarchy to another is using L<Reparent|[%m.elem%]/"Reparent()">.

The dialog is the root of the hierarchy tree. To retrieve the dialog of
any element you can simply call L<GetDialog|[%m.elem%]/"GetDialog()">,
but there are other ways to navigate in the hierarchy tree.

To get all the children of a container call L<GetChild|[%m.elem%]/"GetChild()"> or
L<GetNextChild|[%m.elem%]/"GetNextChild()">. To get just the next
control with the same parent use
L<GetBrother|[%m.elem%]/"GetBrother()">. To get the parent of a
control call L<GetParent|[%m.elem%]/"GetParent()">.
...
The layout size and positioning is automatically updated by L<Map|[%m.elem%]/"Map()">.
L<Map|[%m.elem%]/"Map()"> also updates the dialog layout even if it is already mapped,
so using it or using L<Show|[%m.elem%]/"Show()">, L<ShowXY|[%m.elem%]/"ShowXY()"> or L<Popup|[%m.elem%]/"Popup()"> (they all
call L<Map|[%m.elem%]/"Map()">) will also update the dialog layout. The layout size and
positioning can be manually updated using
L<Refresh|[%m.elem%]/"Refresh()">, even if the dialog is not mapped.
...
L<Refresh|[%m.elem%]/"Refresh()"> for an element inside the dialog to update the dialog
layout. To force a redraw of an element without layout update call
L<Update|[%m.elem%]/"Update()">.
