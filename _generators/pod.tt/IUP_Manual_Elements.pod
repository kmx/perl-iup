...
For more information about using elemets see L<Elements Concept|[%m.intro%]/"Elements Concept">
...
allocated. Each name will point to an internal string.xxxTODOxxx

B<$max_n:> maximum number of names the table can receive. Can be omitted. xxxtodo reuse txt

B<Returns:> the actual number of names loaded to the table. If $names
is C<undef> or $max_n is 0 then returns the maximum number of names.
...
L<Attributes Guide|xxx.html> section.
...
B<$ih:> Identifier of the interface element.
If C<undef> will retrieve from the global environment. xxxcheckthis is it the same as getglobal?
...
See the L<Attributes Guide|xxx.html> for more details.
...
B<Returns:> xxxtodo C<undef> if not found.
...
associated name using L<SetName|/"SetName()"> or using LED or
by C<< name=>'ElemName' >> named parameter of L<new()|/"new()"> constructor.
...
B<$ih:> identifier of the interface element. 
Can be C<undef> if child not C<undef>. xxxcheckthis
...
B<$children:> flag to update its children. xxxcheckthis vales 0 or 1 
...
B<$ih:> Identifier of the interface element. 
If C<undef> will set in the global environment. xxxcheckthis
...
L<StoreAttribute|/"StoreAttribute()xxxremove">.
...
B<$cb_name:> attribute name of the callback. xxxsee

B<$cb_handler:> xxxTODO func refxxx. If C<undef> removes the association.

B<Returns:> the address of the previous function associated to the action xxxTODO ???.

[% h.at_see %]
L<Callback Concept|[%m.intro%]/"xxxCallback Conceptxxx">.
...
B<Returns:> the identifier of the interface element that previously had
the keyboard focus.
...
return value will be different. You could call L<Flush|/"Flush()"> between the
...
B<$ih:> identifier of the interface element.
Use NULL to remove the association. xxxcheckthis

B<$name:> name of the interface element.

B<Returns:> the identifier of the interface element previously associated
...
=head1 Methods common for all Dialog-like elements
...
See the L<PLACEMENT|xxx../dlg/iupdialog.html#PLACEMENT> attribute for
...
B<Returns:> IUP_NOERROR if sucessful. If there was an error returns
IUP_ERROR.
