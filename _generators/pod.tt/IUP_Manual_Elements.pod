...
If left B<detached> it is still necessary to call L<Destroy|/"Destroy()"> to B<
...
xxxcheckthisxxx In Lua, only known internal pointer attributes are returned as
...
iup.GetAttributeData:
 ...
 my $data = $element->GetAttributeData($name);

xxxTODO2xxx consider rename GetAttributeData > GetAttributeSV

xxxTODO1xxx consider introducing SetAttributeSV
...
=item * IUP_CURRENT: use the current position of the dialog. This is
the default value in Lua if the parameter is not defined.
