...
Use L<IUP::SetHandle> or L<IUP::SetAttributeHandle|IUP::SetAttributeHandle> to
...
=item * B<TABTITLE> (non inheritable, at children)

Same as B<< TABTITLE<n> >> but set in each child. 
Works only before the child is added to the tabs. Deprecated, use B<< TABTITLE<n> >>.
...
tabs. Use L<IUP::SetHandle> or L<IUP::SetAttributeHandle> to associate
