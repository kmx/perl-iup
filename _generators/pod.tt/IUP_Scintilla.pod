...
=item * B<X<SAVEDSTATE>SAVEDSTATE> (non inheritable)
...
=item * B<CHARI<id>> (non inheritable, read-only)
...
=item * B<INSERTI<id>> (non inheritable, write-only)
...
=item * B<LINEI<id>> (non inheritable, read-only)
...
=item * B<ANNOTATIONTEXT>B<B<I<id>> >(non inheritable)
...
=item * B<ANNOTATIONSTYLE>B<B<I<id>> >(non inheritable)
...
=item * B<AUTOCSHOWB<I<id>>> (non inheritable, write only)
...
=item * B<BRACEMATCHI<id>> (non inheritable, read only)
...
=item * B<X<CARET>CARET> (non inheritable)
...
=item * B<X<CARETPOS>CARETPOS> (non inheritable)
...
=item * B<FOLDLEVELI<id>> (non inheritable)
...
=item * B<KEYWORDI<id>> (non inheritable, write-only)
...
=item * B<MARGINMASKFOLDERSI<id> >(non inheritable)

defines if a margin is
folding or non-folding. Can be: Yes or No.

=item * B<MARGINSENSITIVEI<id> >(non inheritable)
...
=item * B<MARGINTYPEI<id> >(non inheritable)
...
=item * B<MARGINWIDTHI<id> >(non inheritable)
...
=item * B<MARGINTEXTI<id> >(non inheritable)

controls the text of each line of
a text margin. id is the line number.

=item * B<MARGINTEXTSTYLEI<id> >(non inheritable)
...
=item * B<MARGINCURSORI<id> >(non inheritable)
...
B<MARKERSYMBOLI<id>> (non inheritable)
...
=item * B<MARKERADDI<id>> (non inheritable, write-only)
...
=item * B<MARKERDELETEI<id>> (non inheritable, write-only)
...
=item * B<MARKERGETI<id>> (non inheritable, read-only)
...
=item * B<MARKERNEXTI<id>> (non inheritable, write-only)
...
=item * B<MARKERPREVIOUSI<id>> (non inheritable, write-only)
...
=item * B<MARKERLINEFROMHANDLEI<id>> (non inheritable, read-only)
...
=item * B<STYLEBGCOLORI<id>> (non inheritable)

background color for a style
(See L<Style Definition|#StyleDefinition>). Values in RGB format ("r g
b").

=item * B<STYLEBOLDI<id>> (non inheritable)

the boldness of a font (See
=item * L<Style Definition|#StyleDefinition>).

B<STYLECASEI<id>> (non inheritable)

determines how text is displayed
(See L<Style Definition|#StyleDefinition>). Values: LOWERCASE,
UPPERCASE or MIXED (default).

=item * B<STYLECHARSETI<id>> (non inheritable)
...
=item * B<STYLEFGCOLORI<id>> (non inheritable)

foreground color for a style
(See L<Style Definition|#StyleDefinition>). Values in RGB format ("r g
b").

=item * B<STYLEFONTI<id>> (non inheritable)
...
=item * B<STYLEHOTSPOTI<id>> (non inheritable)
...
=item * B<STYLEVISIBLEI<id>> (non inheritable)

determines if the text is
visible (YES) or hidden (NO) (See L<Style
Definition|#StyleDefinition>).

=item * B<STYLEWEIGHTI<id>> (non inheritable)
...
=item * B<STYLINGI<id>> (non inheritable, write only)
...
=item * B<insert> - =1 when text is inserted, =0 when text is deleted.

=item * B<pos> - 0 based character position when change started.

=item * B<length> - size of the change.

=item * B<text> - the inserted text value. It is NULL when insert=0.
...
3.10.1)
 ...
 int function(Ihandle *ih, int pos, char* text ); [in C]elem:autocselection_cb(pos: number, text: string) -> (ret: number) [in Lua]

=over

=item * B<ih> - identifier of the element that activated the event.

=item * B<pos> - 0 based character start position of the word being completed.

=item * B<text> - the text of the selection.
...
auto-completion list. (since 3.10.1)
 ...
 int function(Ihandle *ih); [in C]elem:autoccancelled_cb() -> (ret: number) [in Lua]
...
auto-completion list was active. (since 3.10.1)
 ...
 int function(Ihandle *ih); [in C]elem:autocchardeleted_cb() -> (ret: number) [in Lua]
...
=item * B<lin, col> - line and column number (start at 0).
...
=item * B<pos> - the character position in the document that corresponds to the
hotspot click.

=item * B<lin> - line in the document that corresponds to the hotspot click.

=item * B<col> - column in the document that corresponds to the hotspot click.

=item * B<status> - status of mouse buttons and certain keyboard keys at the
...
=item * B<margin> - the margin number that was clicked.

=item * B<lin> - line in the document that corresponds to the margin click.
...
=item * B<X<SAVEPOINT_CB>SAVEPOINT_CB>
...
=item * L<SAVEDSTATE|#SAVEDSTATE> attribute.

 int function(Ihandle *ih, int status); [in C]elem

=over

savepoint_cb(status - number) -> (ret: number) [in Lua]

=item * B<ih> - identifier of the element that activated the event.

=item * B<status> - can be 1 (reached) or 0 (left).
...
=item * B<zoomInPoints> - the current zoom factor.
...
=item * L<Drag E<amp> Drop|../attrib/iup_dragdrop.html> callbacks are
supported. See Notes bellow.
...
L<IUP::Text|IUP::Text> auxiliary functions
L<IUP::TextConvertLinColToPos|IUP::TextConvertLinColToPos> and L<IUP::TextConvertPosToLinCol|IUP::TextConvertPosToLinCol> can also
be used.
...
L<IUP::Scintilla|IUP::Scintilla> uses attributes and callbacks very similar to the
L<IUP::Text|IUP::Text> control, except for text formatting.

Although the L<IUP::Scintilla|IUP::Scintilla> documentation should be sufficient for
...
IUP::Scintilla>, so by consulting that documentation you will be able to
