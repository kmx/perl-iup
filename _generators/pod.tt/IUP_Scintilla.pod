=for comment based on iup-3.11 (attributes refreshed to iup-3.32) - http://www.tecgraf.puc-rio.br/iup/en/ctrl/iup_scintilla.html
...
=item * B<MOUSEDWELLTIME> (non inheritable)

time the mouse must sit still, in milliseconds, to generate a
DWELL_CB callback. If set to C<undef>, the default, no dwell events
are generated. (since iup-3.23)
...
=item * B<SAVEPOINT> (non inheritable, write-only)

sets the current state of the document to saved (given value is
ignored). The L<SAVEPOINT_CB> callback is called with status=0 if the
document is modified, and called with status=1 if the save point is
reached with Undo/Redo. After setting the SAVEPOINT, when editing is
done the L<SAVEPOINT_CB> callback is called with status=0. When undo is
performed back to the point were the saved state was set the callback
is called again with status=1. (since iup-3.23)

=item * B<MODIFIED> (non inheritable, read-only)

returns if the text has been modified since the last SAVEPOINT. Can
return Yes or No. (since iup-3.23)
...
=item * B<ADD> (non inheritable, write-only)

Inserts a text at the current position. If VALUELEN is defined, then it
is used instead of strlen. (since iup-3.27)
...
=item * B<CARETCOLOR> (non inheritable)

color of the caret. Values in RGB format ("r g b"). (since iup-3.17)

=item * B<CARETSTYLE> (non inheritable)

style of the caret. Can be LINE, BLOCK or INVISIBLE. Default: LINE.
(since iup-3.17)

=item * B<CARETWIDTH> (non inheritable)

with of the caret line. Can be 0, 1, 2 or 3 pixels. Default: 1. Works
only when CARETSTYLE=LINE. A size of 0 will make the caret invisible
also. (since iup-3.17)

=item * B<CARETLINEVISIBLE> (non inheritable)

Enable or disable caret line background color. Can be Yes or No.
Default: No. (since iup-3.21)

=item * B<CARETLINEBACKCOLOR> (non inheritable)

Color of caret line background. Values in RGB format ("r g b"). (since
iup-3.21)

=item * B<CARETLINEBACKALPHA> (non inheritable)

Set translucency used for line containing the caret. Translucency
ranges from 0 for completely transparent to 255 for opaque with 256
being opaque and not using translucent drawing code which may be
slower. (since iup-3.21)

=item * B<CARETXPOLICY>/B<CARETYPOLICY> (non inheritable)

These set the caret policy. The value can be a combination of "SLOP",
"STRICT", "JUMPS" and "EVEN" using "|" as separator. When SLOP is used
the attribute CARETSLOP defines an unwanted zone for the caret. This
zone is defined as a number of pixels near the vertical margins, and as
a number of lines near the horizontal margins. (since iup-3.26)

=item * B<FIRSTVISIBLELINE> (non inheritable)

the line number of the first visible line. (since iup-3.18)
...
=item * B<SELECTIONFGCOLOR> (non inheritable)

foreground color of the selection. Values in RGB format ("r g b").
Default is system dependent. (since iup-3.23)

=item * B<SELECTIONBGCOLOR> (non inheritable)

background color of the selection. Values in RGB format ("r g b").
Default is system dependent. (since iup-3.23)

=item * B<SELECTIONALPHA> (non inheritable)

The selection can be drawn translucently in the selection background
color by setting an alpha value. Translucency ranges from 0 for
completely transparent to 255 for opaque with 256 being opaque and not
using translucent drawing. (since iup-3.23)
...
=item * B<WORDPOS(id)> (non inheritable, read-only)

returns the start and end of a word in the text around the given
position (in id). It uses the format "B<start>:B<end>". Returns
C<undef> if no words were found. (since iup-3.23)

=item * B<WORDRANGE> (non inheritable)

interval for ISWORD. (since iup-3.23)

=item * B<ISWORD> (non inheritable, read-only)

check if the interval defined by WORDRANGE is a word. (since iup-3.23)
...
=item * B<FOLDLINE(id)> (non inheritable, write-only)

operates over a single line. Can be CONTRACT, EXPAND, or TOGGLE. (since
iup-3.23)

=item * B<FOLDCHILDREN(id)> (non inheritable, write-only)

operates over a single line and all its children. Can be CONTRACT,
EXPAND, or TOGGLE. (since iup-3.23)

=item * B<FOLDALL> (non inheritable, write-only)

operates over all levels. Can be CONTRACT, EXPAND, or TOGGLE. (since
iup-3.23)

=item * B<FOLDEXPANDED(id)> (non inheritable)

the expanded state of a single line. It has no effect on the visible
state of the line or any lines that depend on it. It does change the
markers in the folding margin. If you just want to toggle the fold
state of one line and handle all the lines that are dependent on it, it
is much easier to use TOGGLEFOLD. It can be used to process many folds
without updating the display until you had finished, but you have to
manually hide or show lines, and force a COLORISE. (since iup-3.23)
...
=item * B<FOLDLEVELWHITE(id)> (non inheritable)

the fold level white flag state of a line (indicates that the line is
blank). Can be: "Yes" or "No". (since iup-3.23)

=item * B<FOLDLEVELHEADER(id)> (non inheritable)

the fold level header flag state of a line (indicates that the line is
a header/fold point). Can be: "Yes" or "No". (since iup-3.23)

=item * B<FOLDPARENT(id)> (non inheritable, read-only)

returns the line number of the first line before the given line (in id)
that is marked as a fold point with FOLDLEVELHEADER=Yes and has a fold
level less than the given line. If no line is found, or if the header
flags and fold levels are inconsistent, the return value is -1. (since
iup-3.23)
...
=item * B<ENSUREVISIBLE(id)> (non inheritable, write-only)

ensure that the given line (in id) is visible by expanding folded
parents. Value can be C<undef> or "ENFORCEPOLICY" to apply vertical
caret policy. (since iup-3.23)

=item * B<SHOWLINES> (non inheritable, write-only)

make the given lines interval visible. Value is in the format
"start:end", start default is 0, end default is -1 (last line). (since
iup-3.23)

=item * B<HIDELINES> (non inheritable, write-only)

make the given lines interval hidden. Value is in the format
"start:end", start default is 0, end default is -1 (last line). (since
iup-3.23)
...
=item * B<INDICATORCURRENT> (non inheritable)

sets the indicator number that will affect INDICATORCLEARRANGE and
INDICATORFILLRANGE. (since iup-3.23)

=item * B<INDICATORVALUE> (non inheritable)

sets the indicator value that will affect INDICATORFILLRANGE. (since
iup-3.23)

=item * B<INDICATORCLEARRANGE> (non inheritable, write-only)

clears the current indicator in the range given in the format
"start:length" ("%d:%d" in C). (since iup-3.23)

=item * B<INDICATORFILLRANGE> (non inheritable, write-only)

fills the current indicator with the current value in the range given
in the format "start:length" ("%d:%d" in C). (since iup-3.23)

=item * B<INDICATORSTYLE(id)> (non inheritable)

the style used to draw the given indicator (in id). Can be: PLAIN,
SQUIGGLE, TT, DIAGONAL, STRIKE, HIDDEN, BOX, ROUNDBOX, STRAIGHTBOX,
FULLBOX, DASH, DOTS, SQUIGGLELOW, DOTBOX, SQUIGGLEPIXMAP,
COMPOSITIONTHICK, COMPOSITIONTHIN, TEXTFORE. Default for indicator 0 is
SQUIGGLE, default for indicator 1 is TT, default for indicator 2 is
PLAIN. (since iup-3.23)

=item * B<INDICATORFGCOLOR(id)> (non inheritable)

the color used to draw the given indicator (in id). Value in RGB format
("r g b"). Default for indicator 0 is "0 127 0", default for indicator
1 is "255 0 0", default for indicator 2 is "0 0 255". (since iup-3.23)

=item * B<INDICATOROUTLINEALPHA(id)> (non inheritable)

sets and retrieves the alpha transparency used to draw the outline
color of the given indicator (in id). The alpha value can range from 0
(completely transparent) to 255 (no transparency). (since iup-3.23)

=item * B<INDICATORALPHA(id)> (non inheritable)

sets and retrieves the alpha transparency used to draw the fill color
of the given indicator (in id). The alpha value can range from 0
(completely transparent) to 255 (no transparency). (since iup-3.23)
...
=item * B<COLORISE> (non inheritable, write-only)

requests the lexer to style the document in an character interval given
in value (format "start:end", start default is 0, end default is -1
(last pos)). (since iup-3.23)
...
=item * B<EOL> (non inheritable)

Returns the current end of line character(s). Can be "\r", "\r\n" or
"\n". (since iup-3.24)

=item * B<EOLMODE> (non inheritable)

End of line mode. Can be CR ("\r"), CRLF ("\r\n") or LF ("\n").
Default: LF. (since iup-3.24)

=item * B<EOLVISIBLE> (non inheritable)

End of line visibility. Default: No. (since iup-3.22)

=item * B<FIXEOL> (non inheritable, write only)

Fix the line ends to use only the given mode. Can be CR ("\r"), CRLF
("\r\n") or LF ("\n"). (since iup-3.22, options other than LF since
iup-3.24)
...
=item * B<MARGINMASK(id)> (non inheritable)

defines which markers can be used for symbols in the given margin (id).
The value is a 32 bit number with the selected markers. A single margin
can display several types of markers. See Markers. (since iup-3.22)
...
=item * B<FOLDMARGINCOLOR> (non inheritable, write-only)

changes the color of the fold margin. Values in RGB format ("r g b").
If set to C<undef> reset to internal default values. (since iup-3.22)

=item * B<FOLDMARGINHICOLOR> (non inheritable, write-only)

changes the color of the fold margin highlight. Values in RGB format
("r g b"). If set to C<undef> reset to internal default values. (since
iup-3.22)
...
Prints the text. Use the following attributes to configure the printed
document. Value is the job title. By default will print all text.

=item * B<PRINTDIALOG>

shows the system dialog before printing. Can be YES or NO. Default:
YES. If there is a selection the user can print only the selected text.

=item * B<PRINTMARGINLEFT>

left margin.

=item * B<PRINTMARGINTOP>

top margin.

=item * B<PRINTMARGINRIGHT>

right margin.

=item * B<PRINTMARGINBOTTOM>

bottom margin.

=item * B<PRINTMARGINUNITS>

units used in print margin attributes. Can be: INCH, CM or PIXELS.
Default: INCH.

=item * B<PRINTWORDWRAP>

controls how long lines that don't fit in the page are processed. Can
be: NONE (lines are truncated) or WORD (lines are break between words).
Default: WORD. Notice that CHAR wrap mode is not supported when
printing.

=item * B<PRINTCOLOR>

controls how colors are printed. Can be: NORMAL (print using the
current screen colors), INVERTLIGHT (inverts the light value of all
colors and printing on a white background), BLACKONWHITE (print all
text as black on a white background), COLORONWHITE (prints all text in
its own color on a white background), and COLORONWHITEDEFAULTBG
(everything prints in its own color on a white background except that
line numbers use their own background color). Default: NORMAL.

=item * B<PRINTMAGNIFICATION>

lets you to print at a different size than the screen font. It is the
number of points to add to the size of each screen font. A value of -3
or -4 gives reasonably small print.
...
=item * B<SCROLLBY> (non inheritable, write only)

Scroll the text by the given offsets in the format "lin,col". Positive
lin values increase the line number at the top of the screen (i.e. they
move the text upwards). Positive col values increase the column at the
left edge of the view (i.e. they move the text leftwards). (since
iup-3.17)
...
=item * B<SCROLLWIDTHTRACKING> (non inheritable)

scroll width is adjusted to ensure that all of the lines currently
displayed can be completely scrolled. This mode never adjusts the
scroll width to be narrower. (since iup-3.30)
...
=item * B<TARGETWHOLEDOCUMENT> (non inheritable, write only)

Set the target start to the start of the document and target end to the
end of the document. (since iup-3.23)
...
=item * B<UNDOACTION> (non inheritable, write only)

allows to create a block of undo operations. Can be BEGIN (start a
block) or END (end a block). (since iup-3.21)
...
using the keyboard, undo/redo system or from the clipboard.
 ...
 sub action_handler {
   my ($self, $insert, $pos, $length, $text) = @_;
   #...
 }

B<$self> - reference to the element (L<IUP::Scintilla|IUP::Scintilla>) that activated the event.
...
B<$text> - the inserted text value. It is C<undef> when insert=0.
...
inserted. Automatic insertion can be cancelled by setting the
B<AUTOCCANCEL> attribute before returning from the callback. (since
3.10.1)

 sub autocselection_cb_handler {
   my ($self, $pos, $text) = @_;
   #...
 }
...
auto-completion list was active. (since 3.10.1)
 ...
 sub autocchardeleted_cb_handler {
   my ($self) = @_;
   #...
 }
...
C<ConvertXYToPos> to convert (x,y)
...
B<$lin>, B<$col> - line and column number (start at 0).
...
clicks on text that is in a style with the hotspot attribute set.
 ...
 sub hotspotclick_cb_handler {
   my ($self, $pos, $lin, $col, $status) = @_;
   #...
 }
...
L<BUTTON_CB|[%m.cb%]/BUTTON_CB> can be used for this status.
...
inside a margin that is marked as sensitive.
 ...
 sub marginclick_cb_handler {
   my ($self, $margin, $lin, $status) = @_;
   #...
 }
...
=item * B<INSERTCHECK_CB>

Action generated before text is inserted, allowing the application to
inspect the insertion.

 sub insertcheck_cb_handler {
...
B<$pos> - position where the text will be inserted.

B<$length> - length of the text being inserted.

B<$text> - the text being inserted.

=item * B<LINESCHANGED_CB>

Action generated when the number of lines changes.

 sub lineschanged_cb_handler {
   my ($self, $lin, $linesAdded) = @_;
   #...
 }

B<$self> - reference to the element (L<IUP::Scintilla|IUP::Scintilla>) that activated the event.

B<$lin> - the first affected line.

B<$linesAdded> - the number of lines added (negative when lines were removed).

=item * B<UPDATECONTENT_CB>

Action generated when the document content is updated.

 sub updatecontent_cb_handler {
   my ($self) = @_;
   #...
 }
...
=item * B<UPDATEHSCROLL_CB>

Action generated when the horizontal scroll position is updated.

 sub updatehscroll_cb_handler {
   my ($self) = @_;
   #...
 }
...
=item * B<UPDATESELECTION_CB>

Action generated when the selection is updated.

 sub updateselection_cb_handler {
   my ($self) = @_;
   #...
 }
...
=item * B<UPDATEVSCROLL_CB>

Action generated when the vertical scroll position is updated.

 sub updatevscroll_cb_handler {
   my ($self) = @_;
   #...
 }
...
=item * L<Drag E<amp> Drop|[%m.dnd%]> callbacks are supported. See Notes bellow.
...
The original doc: L<iup_scintilla.html|[%url.iuporigdoc%]elem/iup_scintilla>
[%END%]
