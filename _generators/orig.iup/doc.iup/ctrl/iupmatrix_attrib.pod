...
B<FRAMEVERTCOLORI<L:C>>: Color of the vertical right frame of the cell.
...
B<FRAMEHORIZCOLORI<L:C>>: Color of the horizontal bottom frame of the
...
cell BGCOLOR. Returns NULL if the cell does not exists, or it is not
visible, or the element is not mapped. (since 3.6)

B<X<CELLFGCOLOR>CELLFGCOLORI<L>:I<C>> (read-only): Returns the actual
cell FGCOLOR. Returns NULL if the cell does not exists, or it is not
visible, or the element is not mapped. (since 3.6)
...
"ALEFT" for n=0 and "ACENTER" for nE<gt>0. Not used for the column
...
normal operation mode. Can NOT remove a title column, B<I<C>>E<gt>0.
Ignored if set before map.
...
normal operation mode. Can NOT remove a title line, B<I<L>>E<gt>0.
Ignored if set before map.
...
B<X<MARK>MARKI<L:C>>: marks a cell, a line or a column depending on
MARKMODE, and returns cell, line or column mark state also according to
...
B<CLEARATTRIB> (write-only): Clear all cell attributes if ALL, and all
attributes except titles if CONTENTS. When ALL is specified, all lines
and column attributes are also cleared. (since 3.6)

B<X<CLEARVALUE>CLEARVALUE>B<I<L>:I<C>> (write-only): Clear all cell
...
SORTSIGN* are affected. Action only works if the matrix is mapped and
NOT in callback mode.

B<CLEARVALUE> (write-only): Clear all values if ALL, and all values
except titles if CONTENTS. (since 3.6)
...
B<CLEARVALUE*:I<C >>I<(write-only)>: the values in column C. Its value
defines a line inclusive interval in the "L1-L2" format. The default is
0 and the last line. (since 3.6)

In all cases, values are set to NULL. Action only works if the matrix
is mapped and NOT in callback mode.
...
X<FITTOTEXT>B<FITTOTEXT> (write-only): Fet the RASTERWIDTHn or the
...
ALIGNMENTc, BGCOLOR*, FGCOLOR*, FONT*, VALUE, FRAME*COLOR. Global and
