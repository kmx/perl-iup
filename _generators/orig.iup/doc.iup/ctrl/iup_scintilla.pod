...
IupScintilla library includes the Scintilla 3.4.4 source code, so no
...
=head2 Auto-Completion (since 3.10)

B<AUTOCSHOWB<I<id>>> (non inheritable, write only): causes a list of
...
B<AUTOCCANCEL> (non inheritable, write only): cancels any displayed
...
B<AUTOCACTIVE> (non inheritable, read only): returns YES if there is an
active auto-completion list and NO if there is not.

B<AUTOCPOSSTART> (non inheritable, read only): returns the current
position when the list of words started to be shown.

B<AUTOCCOMPLETE> (non inheritable, write only): triggers
auto-completion. This has the same effect as the tab key.

B<AUTOCSELECT> (non inheritable, write only): selects an item in the
...
B<AUTOCSELECTEDINDEX> (non inheritable, read only): retrieves the
current selection index, set by AUTOCSELECT attribute.

B<AUTOCDROPRESTOFWORD> (non inheritable): when an item is selected, any
word characters following the caret are first erased if this attribute
is set YES. The default is NO.

B<AUTOCMAXHEIGHT> (non inheritable): sets and gets the maximum number
...
B<AUTOCMAXWIDTH> (non inheritable): the maximum width of an
...
B<LOADLEXERLIBRARY> (non inheritable, write-only): Load a Lexer
...
=head2 Search and Replace (since 3.10)

B<SEARCHINTARGET> (non inheritable, write only): This searches for the
...
B<SEARCHFLAGS> (non inheritable): sets and gets the search flags used
...
B<TARGETSTART> (non inheritable): sets and gets the start of target.
...
B<TARGETEND> (non inheritable): sets and gets the end of target. If set
0, target end will be the last position of text.

B<TARGETFROMSELECTION> (non inheritable, write only): set the target
start and end from current position of the selection.

B<REPLACETARGET> (non inheritable, write only): replaces the target
...
B<AUTOCSELECTION_CB>: Action generated when the user has selected an
...
B<AUTOCCANCELLED_CB>: Called after the user has cancelled an
...
B<AUTOCCHARDELETED_CB>: Called after the user deleted a character while
