...
Originally implemented by AndrE<eacute> Clinio.
...
Return: a new color or NULL (nil in Lua) to ignore the change. By
...
Return: If IUP_IGNORE the selection is not accepted. By default the
...
Return: If IUP_IGNORE the switch is not accepted. By default the switch
is always accepted.
