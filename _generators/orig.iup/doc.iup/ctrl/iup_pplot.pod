...
Each data set is added using the L<B<Auxiliary >B<Functions>|#aux>. All
...
----
 ...
 void IupPPlotGetSample(Ihandle *ih, int index, int sample_index, float *x, float *y); [in C]
 void IupPPlotGetSampleStr(Ihandle *ih, int index, int sample_index, const char* *x, float *y);
 iup.PPlotGetSample(ih: ihandle, index, sample_index: number) -> (x, y: number) [in Lua]
 iup.PPlotGetSampleStr(ih: ihandle, index, sample_index: number) -> (x: string, y: number)

Returns the sample value in a dataset at the given B<sample_index>. Can
be used only after the dataset is added to the plot. (since 3.11)
...
B<CANVAS> (read-only) (non inheritable): returns the internal CD canvas
where the draw operation occurs. (since 3.11)

B<REDRAW> (write-only) (non inheritable): redraw all plots and update
the display. All other attributes will B<NOT> update the display, so
you can set many attributes without visual output. Value can be NULL.
If value is "NOFLUSH" rendering is performed internally but display is
not updated. If value is "CURRENT" only the current plot defined by
"PLOT_CURRENT" will be updated and it will behave as "NOFLUSH" (since
3.11).
...
but coordinates are float in plot scale. It is called before the
internal processing, if returns IUP_IGNORE internal processing will not
be done. (since 3.9)
