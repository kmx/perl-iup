...
IupPPlot library includes the PPlot source code so no external
...
To make the control available in Lua, use the initialization function
in C, B<iuppplotlua_open>, after calling B<iuplua_open>. The
iuplua_pplot.h file must also be included in the source code. The
...
Adds a 2D dataset to the plot and returns the dataset index. Redraw is
NOT done until the REDRAW attribute is set. Also it will change the
current dataset index to the return value. You can only set attributes
of a dataset AFTER you added the dataset. Can only be called if
B<IupPPlotBegin> was called. Whenever you create a dataset all its
"DS_*" attributes will be set to the default values. Notice that
DS_MODE must be set before other "DS_*" attributes.
...
Inserts a sample in a dataset. It is used after the dataset is added to
the plot.
...
B<L<FONT|../attrib/iup_font.html>>: the font used in all text elements
...
margin of the dataset area. Default: "15", "15", "30", "15".
...
B<DS_EDIT>: enable or disable the current dataset selection and
...
B<AXS_XTICKDIVISION>, B<AXS_YTICKDIVISION>: number of ticks between
...
B<AXS_XTICKSIZE, AXS_YTICKSIZE>: size of ticks in pixels. Default is 5
...
L<ACTIVE|../attrib/iup_active.html>, L<X|../attrib/iup_x.html>,
L<Y|../attrib/iup_y.html>, L< POSITION|../attrib/iup_position.html>,
...
If the DELETEBEGIN_CB callback returns IUP_IGNORE the delete operation
for all the selected samples are aborted.
...
If the SELECTBEGIN_CB callback returns IUP_IGNORE the selection
...
If the return value is IUP_IGNORE then the sample is not edited. The
application can changed the new value before it is edited.
...
If the EDITBEGIN_CB callback returns IUP_IGNORE the editing operation
is aborted.
