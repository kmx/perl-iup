...
is maintained by Alexey Balakin and available at SourceForge
L<http:E<47>E<47>mathgl.sourceforge.netE<47>|http://mathgl.sourceforge.net/>
with GPL and LGPL licenses.

IupMglPlot library includes the MathGL 2.2 source code, so no external
references are needed, but contains only the LGPL part of the library.
...
Each dataset is added using the L<B<Auxiliary >B<Functions>|#aux>. All
other plot parameters are configured by attributes.
...
B<Linear> sequential data are simply a sequence of points whether in 1D
([y1],[y2],...), 2D ([x1,y1],[x2,y2],...) or 3D
([x1,y1,z1],[x2,y2,z2],...) coordinates.

B<Planar> data is a bi-dimensional array of values, just like a digital
image. Each value is f(x,y), where x belongs to [AXS_XMIN, AXS_XMAX]
and y to [AXS_YMIN, AXS_YMAX].

B<Volumetric> data is a tri-dimensional array of values, that
represents a 3D volume. Each value is f(x,y,z), where x belongs to
[AXS_XMIN, AXS_XMAX] and y to [AXS_YMIN, AXS_YMAX] and z to [AXS_ZMIN,
AXS_ZMAX].

Linear data is the same representation accepted by B<IupPPlot>, with
the addition that B<IupMglPlot> has support for 3D coordinates. When
plotting linear 1D data the automatic range will be XMIN=0 and
XMAX=count-1.

Planar and volumetric data can be very memory consuming, so be careful
when using them. Planar and volumetric data in B<IupMglPlot> have x, y
and z coordinates equidistantly distributed. Planar and volumetric data
are stored in a 1D dataset created by B<IupMglPlotNewDataSet>, and
filled with B<IupMglPlotSetData>, B<IupMglPlotLoadData> or
B<IupMglPlotSetFromFormula> functions only.
...
Zoom can be done using the mouse wheel (Zoom in: scroll down; Zoom out:
scroll up), the Ctrl+Left mouse button pressed and vertical mouse
...
1, 2 or 3. I<B<Linear data only.>>

----

 void IupMglPlotAdd1D(Ihandle* ih, const char* name, double y); [in C]
...
automatically generated (0,1,2,3,...). I<B<Linear data only.>>

----

 void IupMglPlotAdd2D(Ihandle* ih, double x, double y); [in C]
 iup.MglPlotAdd2D(ih: ihandle, x, y: number) [in Lua]

Adds a sample to the dataset. Can only be called if B<IupMglPlotBegin>
was called with dim=2. I<B<Linear data only.>>

----

 void IupMglPlotAdd3D(Ihandle* ih, double x, double y, double z); [in C]
 iup.MglPlotAdd3D(ih: ihandle, x, y, z: number) [in Lua]

Adds a sample to the dataset. Can only be called if B<IupMglPlotBegin>
was called with dim=3. I<B<Linear data only.>>
...
attributes. I<B<Linear data only.>>
...
can be 1, 2 or 3. B<Linear data> can have 1, 2 or 3 dimensions.
B<Planar data> has 2 dimensions, but use dim=1. B<Volumetric data> has
3 dimensions, but use dim=1.

Planar and volumetric data distribute their data in 2D and 3D
continuous arrays, so the number of dimensions is given B<only> when
the array is set in B<IupMglPlotSetData>, B<IupMglPlotLoadData> or
B<IupMglPlotSetFromFormula>. You can convert planar data into linear
data using the DS_REARRANGE and DS_SPLIT attributes.

----

 void IupMglPlotInsert1D(Ihandle *ih, int ds_index, int sample_index, const char** names, double* y, int count); [in C]
...
(0,1,2,3,...). I<B<Linear data only.>>

----

 void IupMglPlotSet1D(Ihandle *ih, int ds_index, const char** names, double* y, int count); [in C]
...
data is automatically generated (0,1,2,3,...). I<B<Linear data only.>>
...
evaluated in the [AXS_?MIN, AXS_?MAX] interval. There is no difference
between lower or upper case in formulas.I< B<Linear data only.>>
...
----
 ...
 void IupMglPlotSetData(Ihandle *ih, int ds_index, double* data, int count_x, int count_y, int count_z); [in C]
...
parameters. If B<count_y> is greater than 1 the data will be B<Planar>
(like a raster 2D image), if B<count_z> is also greater than 1 the data
will be B<Volumetric> (a 3D volume).
...
variables of the formulas must be 'x', 'y' and/or 'z'. Coordinates are
evaluated in the [AXS_?MIN, AXS_?MAX] interval. There is no difference
between lower or upper case in formulas. The operators and functions
can be:
...
----
 ...
 void IupMglPlotTransform(Ihandle* ih, double x, double y, double z, int *ix, int *iy); [in C]
...
----
 ...
 void IupMglPlotTransformTo(Ihandle* ih, int ix, int iy, double *x, double *y, double *z); [in C]
 iup.MglPlotTransformTo(ih: ihandle, ix, iy: number) -> (x, y, z: number) [in Lua]
...
----
 ...
 void IupMglPlotDrawMark(Ihandle* ih, double x, double y, double z); [in C]
...
----
 ...
 void IupMglPlotDrawLine(Ihandle* ih, double x1, double y1, double z1, double x2, double y2, double z2); [in C]
...
----
 ...
 void IupMglPlotDrawText(Ihandle* ih, const char* text, double x, double y, double z); [in C]
...
inside PREDRAW_CB and POSTDRAW_CB callbacks. The attributes DRAWCOLOR,
DRAWFONT, DRAWFONTSTYLE and DRAWFONTSIZE (a factor) can be used to
control text appearance. DRAWFONTSTYLE can be a combination of Plain
(Regular), Bold, Italic (Oblique), Wired, Underline, Strikeout, Left,
Center and Right.

----

 void IupMglPlotPaintTo(Ihandle* ih, const char* format, int w, int h, double dpi, void* data); [in C]
...
support when in OpenGL. Ignored when OPENGL=NO. Default: Yes. When
enabled text has a much better rendering, but 3D graphs will not
...
using custom font files in MathGL font format (*.vfm). The font file
must be located in the path pointed by the "IUP_MGLFONTS" environment
variable, or in the path pointed by the "MGLFONTS" global attribute.
The font name will be automatically combined with the path to compose a
file name. If the font load fail, an internal MathGL font is used. Font
loading is a very slow process in MathGL. B<TrueType (*.ttf) and
OpenType (*.otf) font files support removed in version 3.11.>

All B<*FONTSTYLE> attributes can have a combination of the following
styles: Plain (Regular), Bold, Italic (Oblique), Wired, Underline,
Strikeout, Left, Center and Right.

All B<*FONTSIZE> attributes are actually a factor that multiplies the
size defined by the FONT attribute and an internal MathGL factor. So
the actual size will be fontsize_factor * internal_factor * font_size.
...
All color values can have an optional alpha value. Hexadecimal format
is also accepted.

B<MARGINLEFT, MARGINRIGHT, MARGINTOP, MARGINBOTTOM >(non inheritable):
internal MathGL margin configuration of the dataset area. Can be Yes or
No. Default: "Yes". (since 3.11)

B<MGLGRAPH> (read-only)(non inheritable): returns the B< mglGraph>*
handle used internally. (since 3.11)
...
to B<IupUpdate>, it will have the same effect as if REDRAW was set.
...
B<TITLEFONTSIZE>: title font size factor. Default: 1.0. The internal
MathGL factor is 2.0.

B<TITLEFONTSTYLE>: title font style.
...
B<LEGENDFONTSIZE>: legend font size factor. Default: 1.0. The internal
MathGL factor is 0.8.

B<LEGENDFONTSTYLE>: legend font style.
...
"CONTINUOUS" (or "SOLID"), "LONGDASHED", "DASHED", "SMALLDASHED",
"DOTTED", "DASH_DOT", "SMALLDASH_DOT". Default is "CONTINUOUS".

B<DS_LINEWIDTH>: line width of the current dataset. Default: 1. Can be
1 to 9 (since 3.11).

B<DS_MARKSTYLE>: mark style of the current dataset. Can be: "PLUS",
"STAR", "CIRCLE", "X", "BOX", "DIAMOND", "HOLLOW_CIRCLE", "HOLLOW_BOX",
"HOLLOW_DIAMOND", "DOT", "ARROW_UP", "ARROW_LEFT", "ARROW_DOWN",
"ARROW_RIGHT". Default is "X".

B<DS_MARKSIZE>: mark size of the current dataset in normalized
coordinates. Default: 1.0.
...
B<AXS_?FONTSIZE>: axis label font size factor. Default: 0.8. The
internal MathGL factor is 1.4.

B<AXS_?FONTSTYLE>: axis label font style.
...
when AUTOMIN or AUTOMAX are enabled. Default: 1.0 and -1.0. When set
will set the correspondent AUTOMIN or AUTOMAX to NO (since 3.11).
...
B<AXS_?TICKFONTSIZE>: axis tick number font size factor. Default: 0.8.
The internal MathGL factor is 1.0.

B<AXS_?TICKFONTSTYLE>: axis tick number font style.
...
rotation along the 3D axis. Can be YES or NO. Default: YES (NO for Y
axis). This means that text will be aligned with the axis even when
rotating the graph. When NO text will be horizontal, always facing the
camera, independently from graph rotation. For 2D graphs is useful to
the Y axis only.
...
Selection and editing of a dataset using the DS_EDIT attribute are not
implemented.
...
parameter. *FONTSIZE attributes are a multiple factor of the FONT size.
DASH_DOT_DOT line style is not supported, but has new line styles:
LONGDASHED, SMALLDASHED and SMALLDASH_DOT. AXS_?TICKMAJORSIZE, MARKSIZE
are in normalized coordinates. New options for GRID: Z, XYZ, XZ, YZ.
AXS_?TICKSIZE renamed to AXS_?TICKMINORSIZE, and is a factor of the
AXS_?TICKMAJORSIZE. AXS_?TICKMAJORSPAN default value is -5.
AXS_?TICKFORMAT default is internally computed according to the Min-Max
range.
...
=head2 X<KnownIssues>Known Issues/To Do
...
=item * B<Text render quality is lower than in IupPPlot.>

=item * Logarithm scale is not working properly.

=item * Automatic ticks computation needs to be improved.

=item * Text rotation when DS_SHOWVALUES=Yes is not ok. (MathGL)

=item * When OPENGL=Yes initial size is smaller. (MathGL)

=item * B<There is still lots of MathGL features not available in
IupMglPlot.>
