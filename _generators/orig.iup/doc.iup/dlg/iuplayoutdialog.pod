...
constructed with other IUP elements inside. The dialog can be shown
with any of the show functions B<IupShow>, B<IupShowXY> or B<IupPopup>.
...
B<dialog>: identifier of the dialog to display the layout.
...
when they were changed by the application.
