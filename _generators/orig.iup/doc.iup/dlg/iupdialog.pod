...
L<Y|../attrib/iup_y.html>, L<WID|../attrib/iup_wid.html>,
...
Motif the decorations are controled by the Window Manager and may not
...
windowing system will not be able to change the size beyond this limit.
Default: 1x1. (since 3.0)

B<MODAL> (read-only): Returns the popup state. It is "YES" if the
dialog was shown using IupPopup. It is "NO" if IupShow was used or it
is not visible. (since 3.0)
...
size. Default: YES. In Motif the decorations are controlled by the
...
screen. The coordinates are the same as the L<X|../attrib/iup_x.html>
and L<Y|../attrib/iup_y.html> attributes. (since 3.0)
...
opened, minimized or restored from a minimization.
