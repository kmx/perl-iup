...
=item * IupTree, IupList, IupPPlot and IupMatrix improvements
...
=item * Debian and LuaRocks packages
...
=item * B<Priority:> Images in B< IupList>.

=item * B<Priority:> Lua Rocks and Debian distribution packages.

=item * B<Priority:> Unicode Support (UTF-8) in Windows and Motif.

=item * Drag&Drop between controls.

=item * A tutorial section in the documentation. One big Controls Demo
just like GTK, wxWidgets and Qt have.

=item * A MacOS X native driver using Carbon or Cocoa.

=item * B<IupGLCanvas> for MacOSX.
...
=item * Support for saving global mouse and keyboard events, so they
...
=item * Known Issue: the MDI Window menu is not properly updated when
the child mdi is closed. (since 3.0)
...
=item * B<Priority: >mark using the keyboard.

=item * B<Priority:> drag&drop of columns position.

=item * Merge cells.

=item * Other columns/lines fixed at start like column 0 and line 0.
...
=item * B<Priority:> check mark for nodes.
...
=item * B<Priority: > Support for inserting a new point in an existing
dataset programmatically and interactively.

=item * B<Priority: > Allow to select multiple nodes at once dragging
the mouse within a region.
...
=item * PPlot has a callback mode feature that is not available in
IupPPlot.
