...
make the controls available in Lua, use the initialization function in
C, B<iupcontrolslua_open>, after calling B<iuplua_open>.

Your application must be linked to the CPI control library
(B<iupcontrols>), the CD_IUP driver (B<iupcd>), and with the
L<CD|http://www.tecgraf.puc-rio.br/cd> library (B<cd>). To use its
bindings to Lua, the program must also be linked to the
