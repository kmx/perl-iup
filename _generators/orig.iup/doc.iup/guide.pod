...
enable Windows XP Visual Styles. B<iup.rc> is located in "/etc" folder
of the distribution.
...
type "make" in the command line on the main IUP folder and all
libraries and executables will be build. Set the TECTOOLS_HOME
environment variable to the folder were the CD, IM and Lua libraries
...
easy to understand also. In IUP's main directory, and in each source
directory, there are files named I<make_uname.bat> that build the
libraries using B<Tecmake>. To build for Windows using Visual C 7.0
(2005) for example, just execute I<"make_uname vc7"> in the iup root
folder, or the DLLs with Visual C++ 9 (2008) type I< "make_uname
dll9">. The Visual Studio workspaces with the respective projects
available in the source package is for debugging purposes only.
...
want installed, see the documentation bellow.
...
LUALIB and LUABINDIR definitions before using Tecmake.
...
=head2 L<IupEu|http://jeremy.cowgar.com/cgi-bin/wiki/IupEu> by Jeremy
Cowgar

=over

Iup wrapped for L<Euphoria|http://www.rapideuphoria.com/>.
