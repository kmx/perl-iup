...
B<libiupimglib>.a on Unix).

The library is large and could increase the size of the executable. To
avoid linking with the library get the source code and extract only the
image you need.
...
name are NOT registered using B<IupSetHandle>, they will be
