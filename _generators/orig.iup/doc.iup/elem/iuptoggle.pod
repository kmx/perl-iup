...
only when XP Style is active. Text is always left aligned. (since 3.0)
...
toggle. In Windows, when using XP Style FGCOLOR is ignored. Default:
the global attribute DLGFGCOLOR.
...
state.
 ...
 int funcion(Ihandle* ih, int state); [in C]elem:action(state: number) -> (ret: number) [in Lua]
