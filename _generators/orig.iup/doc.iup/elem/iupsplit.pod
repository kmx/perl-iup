...
Creates a container that split its client area in two. Allows the
...
It does not have a native representation but it uses a B<IupCanvas> to
...
B<CLIENTSIZE1>, B<CLIENTSIZE2>: Same as
L<CLIENTSIZE|../attrib/iup_clientsize.html>, but one for each of the
child areas.

B<DIRECTION> (creation only): Indicates the direction of the resize and
the position of the bar handler. Possible values are "VERTICAL" or
