...
B<CARET> (non inheritable): Position of the insertion point. Its format
...
B<CARETPOS> (non inheritable): Also the position of the insertion
point, but using a zero based character unique index "pos". Useful for
indexing the VALUE string. See the Notes bellow if using UTF-8 strings
in GTK. (since 3.0)
...
B<NC>: Maximum number of characters allowed for keyboard input, larger
...
CARET attribute ("lin:col" or "col" starting at 1). (since 3.0)
...
B<X<SELECTION>SELECTION> (non inheritable): Selection interval. Returns
...
string. The values ALL and NONE are also accepted. See the Notes bellow
...
control. The horizontal scrollbar is removed. Default: NO.
...
Returns: IUP_IGNORE is processed in Windows and Motif. It is ignored in
GTK.
