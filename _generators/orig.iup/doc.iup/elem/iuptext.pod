...
B<COUNT> (read-only): returns the number of characters in the text.
(since 3.5)
...
B<LINECOUNT> (read-only): returns the number of characters in the line
...
CARET attribute ("lin:col" or "col" starting at 1). In Windows, when
FORMATTING=Yes "col" is ignored. (since 3.0)
...
the CARETPOS attribute ("pos" starting at 0). In Windows, when
FORMATTING=Yes "col" is ignored. (since 3.0)
