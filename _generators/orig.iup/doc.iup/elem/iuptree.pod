...
B<DRAGDROP_CB>: Action generated when a drag & drop is executed.
...
modified. For example, a id=2 will always refer to the third node in
the tree. For that reason, there is also the B<USERDATAid>, which
allows identifying a specific node. The B<USERDATAid >always refers to
the same node. The B< USERDATAid> may contain a user-created structure
with other information for the node.
...
Is the IupTree default operation mode (MARKMODE=SINGLE). In this mode
...
It is equivalent of retrieving the B<FINDUSERDATAp> attribute.
...
But the attribute string is complemented with the id value. For ex:
 ...
 IupTreeSetAttribute(ih, "KIND" , 30, value) = IupSetAttribute(ih, "KIND30", value)
 IupTreeSetAttribute(ih, "ADDLEAF" , 10, value) = IupSetAttribute(ih, "ADDLEAF10", value)
...
for the IupTree with some limitations.
