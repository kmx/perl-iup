...
must be used to define the end of the list in C. It can be empty in C
or Lua, not in LED.
...
B<ALIGNMENT> (non inheritable): Vertically aligns the elements.
...
vertically and to fully occupy its space available inside the box.
Default: "NO". This has the same effect as setting EXPAND=VERTICAL on
...
horizontal space. The space width will be based on the largest child.
