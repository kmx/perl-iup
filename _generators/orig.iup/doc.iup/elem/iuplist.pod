...
=item * Items after "n+1" are ignored. (since 3.0)
...
traversal of the control. Default: YES. (since 3.0)
...
B<DRAGDROP> [Windows and GTK Only] (non inheritable): Enable or disable
...
B<INSERTITEMn> (write-only): inserts an item before the given n
...
B<VISIBLE_ITEMS>: Number of items that are visible when DROPDOWN=YES is
used for the dropdown list. Default: 5. Ignored in GTK.
...
L<EXPAND|../attrib/iup_expand.html>, L<X|../attrib/iup_x.html>,
...
changed item:
 ...
 int function (Ihandle *ih, char *text, int pos, int state); [in C]elem:action(text: string, pos, state: number) -> (ret: number) [in Lua]

B<ih>: identifier of the element that activated the event.

B<text>: Text of the changed item.

B<pos>: Number of the changed item.
...
Called only when DROPDOWN=NO. (since 3.0)
 ...
 int function (Ihandle *ih, int pos, char *text); [in C]elem:action(pos: number, text: string) -> (ret: number) [in Lua]

B<ih>: identifier of the element that activated the event.

B<pos>: Number of the selected item.

B<text>: Text of the selected item.

B<DROPDOWN_CB>: Action generated when the list of a dropdown is shown
and hidden. Called only when DROPDOWN=YES. (since 3.0)
...
cursor is near the control border. Also the selection and caret
attributes are not preserved if the list loses its focus, or in other
words these attributes are only useful in Windows if the list has the
focus.
