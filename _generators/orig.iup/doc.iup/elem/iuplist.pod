...
B<IMAGEid> (non inheritable): image name to be used in the specified
item, where id is the specified item starting at 1. The item must
already exist. Use L<IupSetHandle|../func/iupsethandle.html> or
...
box. Images don't need to have the same size. (since 3.6)
...
B<SHOWIMAGE> (creation only): enables the use of an image for each
item. Can be "YES" or "NO". Ignored if set before map. (since 3.6)
