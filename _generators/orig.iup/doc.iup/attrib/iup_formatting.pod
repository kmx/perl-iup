...
Removes the formatting of the current selection. The given value is
...
B<SELECTION>: same as the IupText
L<SELECTION|../elem/iuptext.html#SELECTION> attribute. If not defined
the B<IupText> attribute will be used. If the B<IupText> attribute is
also not defined then the current position will receive the format, so
new text inserted or typed will be formatted with the tag (this is not
working in GTK). Different tags that use the same selection interval
are combined.
...
B<SPACEBEFORE>: distance left empty bellow the paragraph.
