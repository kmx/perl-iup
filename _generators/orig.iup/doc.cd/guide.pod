...
"make" in the command line on the "src" folder and all libraries will
be build. Set the TECTOOLS_HOME environment variable to the folder were
the IM and Lua libraries are installed, by default it will assume
"TECTOOLS_HOME=../..".
...
to understand also. There are files named I<make_uname.bat> that build
the libraries using B<Tecmake>. To build for Windows using Visual C 9.0
(2008) for example, just execute I<"make_uname vc9"> , or for the DLLs
type I<"make_uname dll9">. The Visual Studio workspaces with the
...
LUA_INC, LUA_LIB and LUA_BIN before using Tecmake or Tecmake Compact.
