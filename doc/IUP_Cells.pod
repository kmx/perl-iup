...
<p>
  <table border="1">
    <tbody align="center">
      <tr>
        <th style="text-align:center;">Checkerboard Pattern</th>
        <th style="text-align:center;">Numbering Cells</th>
      </tr>
      <tr>
        <td style="vertical-align:top;padding:10;"><img src="http://kmx.github.com/perl-iup/img-3.3/ctrl/iupcells_chess.jpg"></td>
        <td style="vertical-align:top;padding:10;"><img src="http://kmx.github.com/perl-iup/img-3.3/ctrl/iupcells_num.jpg"></td>
      </tr>
    </tbody>
  </table>
</p>
...
B<IUP::Cells> class is used in the following sample scripts:

=over

=item * L<sample1.pl|http://www.x.cz/samples/x.pl> - 'Checkerboard Pattern' example

=item * L<sample2.pl|http://www.x.cz/samples/x.pl> - 'Numbering Cells' example

=item * L<sample3.pl|http://www.x.cz/samples/x.pl> - sample application using zzz
...
B<Note:> You can pass to C<new()> other C<ATTRIBUTE=E<gt>'value'> pairs relevant
to this element - see L<IUP::Manual::02_CreatingElements|IUP::Manual::02_CreatingElements>.

B<Returns> - the identifier of the created element, or C<undef> if an error
occurs.
...
=item * B<CANVAS> (read-only, non inheritable)

Returns the internal CD canvas xxx???. This attribute should be used only in specific cases and by
experienced IUP::Canvas programmers.
...
=item * B<FIRST_COL> (read-only, non inheritable)

Returns the number of the first visible column.

=item * B<FIRST_LINE> (read-only, non inheritable)

Returns the number of the first visible line.

=item * B<FULL_VISIBLE> (write-only, non inheritable)
...
=item * B<IMAGE_CANVAS> (read-only, non inheritable)

Returns the internal
image CD canvas xxx???. This attribute should be used only in specific cases
and by experienced CD programmers.

=item * B<LIMITS(L):(C)> (read-only, non inheritable)
...
=item * B<REPAINT> (write-only, non inheritable)
...
B<$button, $pressed, $x, $y, $status> - same as the L<BUTTON_CB|IUP::Manual::04_Callbacks/BUTTON_CB>
by IUP::Canvas xxx??? callback.
...
B<$x, $y, $status> - same as the L<MOTION_CB|IUP::Manual::04_Callbacks/MOTION_CB>
by IUP::Canvas xxx??? callback.
