=for comment based on iup-3.5 (attributes refreshed to iup-3.32) - http://www.tecgraf.puc-rio.br/iup/en/dlg/iupdialog.html
...
I<(non inheritable) (read-only)>

Returns the border size. (since iup-3.18)
...
position controls outside the client area. Format "I<dx>xI<dy>", where
I<dx> and I<dy> are integer values corresponding to the horizontal and
vertical offsets, respectively, in pixels. Default: 0x0. (since iup-3.14)
...
Same as L<ACTIVE|IUP::Manual::03_Attributes/ACTIVE> but does not affect the controls
inside the dialog. (since iup-3.13)
...
To associate a name to element use L<SetName|IUP::Manual::02_Elements/"SetName()"> or C<< name=>'ElemName' >> named parameter of L<new()|IUP::Manual::02_Elements/"new()"> constructor.
...
Do not set focus after show. (since iup-3.30)
...
Allows the application to customize the dialog frame elements (the title
and its buttons) by using IUP controls for its elements like caption,
minimize button, maximize button, and close buttons. The custom frame
support uses the native system support for custom frames. The
application is responsible for leaving space for the borders. One
drawback is that menu bars will not work. For the dialog to be able to
be moved an L<IUP::Label|IUP::Label> or an L<IUP::Canvas|IUP::Canvas>
must be at the top of the dialog and must have the NAME attribute set to
CUSTOMFRAMECAPTION. Native custom frames are supported only in Windows
and in GTK version 3.10. See the Custom Frame notes below. (since
iup-3.18)

=item B<MAXIMIZED>

I<[Windows and GTK Only] (read-only)>

Indicates if the dialog is maximized. Can be YES or NO. (since iup-3.12)

=item B<MINIMIZED>

I<[Windows and GTK Only] (read-only)>

Indicates if the dialog is minimized. Can be YES or NO. (since iup-3.15)
...
=item B<TASKBARPROGRESS>

I<[Windows Only] (write-only)>

This functionality enables the use of a progress bar on a taskbar button
(Windows 7 or earlier version) (available only for Visual C++ 10 and
above). Default: NO. (since iup-3.10)

=item B<TASKBARPROGRESSSTATE>

I<[Windows Only] (write-only)>

Sets the type and state of the progress indicator displayed on a taskbar
button. Possible values: NORMAL (a green bar), PAUSED (a yellow bar),
ERROR (a red bar), INDETERMINATE (a green marquee) and NOPROGRESS (no
bar). Default: NORMAL. (since iup-3.10)

=item B<TASKBARPROGRESSVALUE>

I<[Windows Only] (write-only)>

Updates a progress bar hosted in a taskbar button to show the specific
percentage completed of the full operation. The value must be between 0
and 100. (since iup-3.10)

=item B<TASKBARBUTTON>

I<[Windows Only]>

If set to SHOW force the application button to be shown on the taskbar
even if the dialog does not have decorations. If set to HIDE force the
application button to be hidden from the taskbar, but also in this case
the system menu, the maximize and minimize buttons will be hidden.
(since iup-3.28)
...
Sets an RGBA image as the dialog background so it is possible to create a
non rectangle window with transparency, but it can not have children.
Used usually for splash screens. It must be set before map so the native
window would be properly initialized when mapped. Works also for GTK but
as the SHAPEIMAGE attribute. (since iup-3.16)

=item B<SHAPEIMAGE>

I<[Windows and GTK Only]>

Sets an RGBA image as the dialog shape so it is possible to create a non
rectangle window with children. (GTK 2.12) Only the fully transparent
pixels will be transparent. The pixels colors will be ignored, only the
alpha channel is used. (since iup-3.26)
...
=item B<TRAYTIPMARKUP>

I<[GTK Only]>

Allows the tip string to contain Pango markup commands. Can be "YES" or
"NO". Default: "NO". Must be set before setting the TRAYTIP attribute.
(GTK 2.16) (since iup-3.6)
...
I<[GTK Only] (non inheritable)>

Hides the title bar with all its elements. (since iup-3.20) (GTK 3.10)
...
=item B<CUSTOMFRAMEDRAW>

I<[Windows Only] (non inheritable)>

Allows the application to customize the dialog frame elements (the title
and its buttons) by drawing them with the CUSTOMFRAMEDRAW_CB callback.
Can be Yes or No. The Window client area is expanded to include the
whole window. Notice that the dialog attributes like BORDER, RESIZE,
MAXBOX, MINBOX and TITLE must still be defined. But maximize, minimize
and close buttons must be manually implemented in the BUTTON_CB
callback. One drawback is that menu bars will not work. (since iup-3.18)

=item B<CUSTOMFRAMECAPTIONHEIGHT>

I<[Windows Only] (non inheritable)>

Height of the caption area. If not defined it will use the system size.
(since iup-3.18)

=item B<CUSTOMFRAMECAPTIONLIMITS>

I<[Windows Only] (non inheritable)>

Limits of the caption area at left and at right. The caption area is
always expanded inside the limits when the dialog is resized. Format is
"left:right". Default: "0:0". This will allow the dialog to be moved by
the system when the user click and drag the caption area. If not defined
but CUSTOMFRAMECAPTION is defined, then it will use the caption element
horizontal position and size for the limits. (since iup-3.18)
...
=item B<MAXIMIZEATPARENT>

I<[Windows Only]>

When using multiple monitors, maximize the dialog in the same monitor
that the parent dialog is. (since iup-3.28)
...
=item B<CUSTOMFRAME_CB>

I<[Windows Only]>

Called when the dialog must be redrawn. Although it is designed for
drawing the frame elements, all the dialog must be painted. Works only
when CUSTOMFRAME or CUSTOMFRAMEDRAW is defined. The dialog can be used
just like an L<IUP::Canvas|IUP::Canvas> to draw its elements, the
HDC_WMPAINT and CLIPRECT attributes are defined during the callback. For
mouse callbacks use the same callbacks as L<IUP::Canvas|IUP::Canvas>,
such as BUTTON_CB and MOTION_CB. (since iup-3.18)

B<Callback handler prototype:>

 sub customframe_cb_handler {
   my ($self) = @_;
   #...
 }
...
=item * L<0-basic/button.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/button.pl> - IUP::Button example

=item * L<0-basic/canvas1.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/canvas1.pl> - IUP::Canvas example

=item * L<0-basic/canvas2.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/canvas2.pl> - IUP::Canvas example

=item * L<0-basic/canvas3.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/canvas3.pl> - IUP::Canvas example

=item * L<0-basic/cbox.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/cbox.pl> - IUP::Cbox example

=item * L<0-basic/cells_checkboard.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/cells_checkboard.pl> - IUP::Cells example

=item * L<0-basic/cells_degrade.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/cells_degrade.pl> - IUP::Cells example

=item * L<0-basic/cells_numbering.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/cells_numbering.pl> - IUP::Cells example

=item * L<0-basic/colorbar.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/colorbar.pl> - IUP::ColorBar example

=item * L<0-basic/colorbrowser.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/colorbrowser.pl> - IUP::ColorBrowser example

=item * L<0-basic/dial.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/dial.pl> - IUP::Dial example

=item * L<0-basic/dialog1.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/dialog1.pl> - IUP::Dialog example

=item * L<0-basic/dialog2.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/dialog2.pl> - IUP::Dialog example

=item * L<0-basic/drag-demo.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/drag-demo.pl> - IUP::Text DRAG-and-DROP demo (drag part)

=item * L<0-basic/expander.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/expander.pl> - IUP::Expander example

=item * L<0-basic/fill.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/fill.pl> - IUP::Fill Example

=item * L<0-basic/frame1.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/frame1.pl> - IUP::Frame Example

=item * L<0-basic/frame2.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/frame2.pl> - IUP::Image example

=item * L<0-basic/gauge.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/gauge.pl> - IUP::Gauge example

=item * L<0-basic/glcanvas1.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/glcanvas1.pl> - IUP::CanvasGL example

=item * L<0-basic/glcanvas2.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/glcanvas2.pl> - IUP::CanvasGL example

=item * L<0-basic/glcanvasbox.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/glcanvasbox.pl> - warn "ACTION: x=$x y=$y\n";

=item * L<0-basic/glcontrols.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/glcontrols.pl> - original: http://webserver2.tecgraf.puc-rio.br/iup/examples/C/glcontrols.c

=item * L<0-basic/gridbox.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/gridbox.pl> - IUP::GridBox example

=item * L<0-basic/hbox.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/hbox.pl> - IUP::Hbox example

=item * L<0-basic/idle.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/idle.pl> - IUP->SetIdle Example

=item * L<0-basic/image.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/image.pl> - IUP::Image example

=item * L<0-basic/item.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/item.pl> - IUP::Item Example

=item * L<0-basic/label.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/label.pl> - IUP::Label and IUP::Link example

=item * L<0-basic/layoutdialog.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/layoutdialog.pl> - IUP::LayoutDialog example

=item * L<0-basic/list1.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/list1.pl> - IUP::List example

=item * L<0-basic/list2.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/list2.pl> - IUP::List example

=item * L<0-basic/mask_matrix.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/mask_matrix.pl> - IUP::Matrix + MASK example

=item * L<0-basic/mask_simple.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/mask_simple.pl> - IUP::Text + MASK example

=item * L<0-basic/matrix_basic.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/matrix_basic.pl> - IUP::Matrix example

=item * L<0-basic/matrix_cbmode.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/matrix_cbmode.pl> - IUP::Matrix (callback mode) example

=item * L<0-basic/matrixlist.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/matrixlist.pl> - IUP::MatrixList example

=item * L<0-basic/menu.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/menu.pl> - IUP::Menu example

=item * L<0-basic/message_clone.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/message_clone.pl>

=item * L<0-basic/mglplot1.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/mglplot1.pl> - IUP::MglPlot example
...
=item * L<0-basic/pre-dialogs.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/pre-dialogs.pl> - IUP::Expander example

=item * L<0-basic/progressbar1.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/progressbar1.pl> - IUP::Progressbar example

=item * L<0-basic/progressbar2.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/progressbar2.pl> - IUP::ProgressBar example

=item * L<0-basic/progressbar3.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/progressbar3.pl> - IUP::ProgressBar example

=item * L<0-basic/radio.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/radio.pl> - IUP::Radio example

=item * L<0-basic/sbox1.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/sbox1.pl> - IUP::Sbox example

=item * L<0-basic/sbox2.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/sbox2.pl> - IUP::Sbox example

=item * L<0-basic/scintilla.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/scintilla.pl> - IUP::Scintilla example

=item * L<0-basic/scrollbar.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/scrollbar.pl> - IUP::Canvas (with scrollbar) example

=item * L<0-basic/separator.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/separator.pl> - IUP::Separator example

=item * L<0-basic/submenu.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/submenu.pl> - IUP::Submenu example

=item * L<0-basic/tabs1.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/tabs1.pl> - IUP::Tabs example 

=item * L<0-basic/tabs2.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/tabs2.pl> - IUP::Tabs example

=item * L<0-basic/tabs3.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/tabs3.pl> - IUP::Tabs example 

=item * L<0-basic/text1.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/text1.pl> - IUP::Text (single line) example

=item * L<0-basic/text2.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/text2.pl> - IUP::Text example

=item * L<0-basic/text_font.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/text_font.pl> - IUP::Text (font) example

=item * L<0-basic/text_format.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/text_format.pl> - IUP::Text (formating) example

=item * L<0-basic/text_multiline.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/text_multiline.pl> - IUP::Text (multiline) example

=item * L<0-basic/timer.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/timer.pl> - IUP::Timer example

=item * L<0-basic/toggle.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/toggle.pl> - IUP::Toggle example

=item * L<0-basic/tray.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/tray.pl> - IUP::Dialog + TRAY / TRAYTIP / TRAYIMAGE example

=item * L<0-basic/tree1.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/tree1.pl> - IUP::Tree example (animals) - using helper methods e.g. TreeAddNodes()

=item * L<0-basic/tree2.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/tree2.pl> - IUP::Tree example (figures) - using attribute only interface

=item * L<0-basic/tree3.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/tree3.pl> - IUP::Tree example (figures) - using attribute only interface

=item * L<0-basic/tree_set_attrs.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/tree_set_attrs.pl> - IUP::Tree example (attributes)

=item * L<0-basic/val.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/val.pl> - IUP::Val example

=item * L<0-basic/vbox.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/vbox.pl> - IUP::Vbox example

=item * L<0-basic/zbox.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/zbox.pl> - IUP::Zbox example

=item * L<1-apps/app-mdi.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/1-apps/app-mdi.pl> - IUP app example
...
=item * L<1-apps/app-sample1.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/1-apps/app-sample1.pl> - example used for screenshot - IUP.pod

=item * L<1-apps/app-sample2.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/1-apps/app-sample2.pl> - example based on the original sample.c

=item * L<1-apps/app-simple-demo.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/1-apps/app-simple-demo.pl> - example used for screenshot - IUP.pod
