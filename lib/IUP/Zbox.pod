...
L<SetName|IUP::Manual::02_Elements/"SetName()">. In perl a name is always
automatically created, but you can change it later. xxx-check-this
...
The visible child accessed by its name. The value passed must be
the perl reference or name of one of the children contained in the zbox.

Use C<< $child->SetName('...') >> or C<< $child = IUP::AnyElement->new( name=>'...' ) >>
to associate a child to a name. XXX-test-this
