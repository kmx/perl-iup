=for comment based on iup-3.5 (attributes refreshed to iup-3.32) - http://www.tecgraf.puc-rio.br/iup/en/elem/iupzbox.html
...
Compute the natural size using all children. If set to NO will compute using only the visible child. Default: Yes. (since iup-3.27)
