=for comment based on iup-3.9 (attributes refreshed to iup-3.32) - http://www.tecgraf.puc-rio.br/iup/en/ctrl/iupgauge.html
...
Color of the background inside the borders. Predefined to "220 220 220".
Can be C<undef>. When C<undef> it will use the parent's background color. (since iup-3.28)

=item B<CANFOCUS>

Enables the focus traversal of the control. Default: NO.
(different from L<IUP::Canvas|IUP::Canvas>)
...
Use a 1 pixel flat border instead of the default 3 pixels sunken border.
Can be Yes or No. Default: No. (since iup-3.21)

=item B<FLATCOLOR>

Color of the border when FLAT=Yes. Default: "160 160 160". (since iup-3.21)
...
Width and height are swapped when orientation is set. (since iup-3.27)
...
Same as PADDING but using the units of the SIZE attribute.
It will actually set the PADDING attribute. (since iup-3.29)
...
Indicates if the text inside the Gauge is to be shown or not.
If the gauge is dashed the text is never shown. Possible values:
"YES" or "NO". Default: "YES".
