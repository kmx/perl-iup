=for comment based on iup-3.5 (attributes refreshed to iup-3.32) - http://www.tecgraf.puc-rio.br/iup/en/elem/iupnormalizer.html
...
I<(non inheritable, write-only)> Removes a control from the normalizer. The value passed must be the name of an element.

To associate a name to element use L<SetName|IUP::Manual::02_Elements/"SetName()"> or C<< name=>'ElemName' >> named parameter of L<new()|IUP::Manual::02_Elements/"new()"> constructor. (since iup-3.17)
