...
=item * B<NORMALIZERGROUP> (non inheritable, for contained controls use)
...
Adds a control to the normalizer. The value passed must be the perl reference or name of the element.

To associate a name to element use L<SetName|/"SetName()"> or C<< name=>'ElemName' >> named parameter of L<new()|/"new()"> constructor.

=item * B<ADDCONTROL_HANDLE> (non inheritable)
