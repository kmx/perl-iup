=for comment based on iup-3.2 (attributes refreshed to iup-3.32) - http://www.tecgraf.puc-rio.br/iup/en/elem/iupbutton.html
...
Enables the focus callback forwarding to the next native parent with FOCUS_CB defined. Default: NO. (since iup-3.23)
...
Same as PADDING but using the units of the B<SIZE> attribute. It will actually set the PADDING attribute. (since iup-3.29)
...
Same as SPACING but using the units of the vertical part of the B<SIZE> attribute. It will actually set the SPACING attribute. (since iup-3.29)
