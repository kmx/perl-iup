=for comment based on iup-3.5 (attributes refreshed to iup-3.32) - http://www.tecgraf.puc-rio.br/iup/en/dlg/iupfiledlg.html
...
Default extension to be used if the selected file does not have an
...
Do not show the places bar. (since iup-3.26)
...
Show an edit box in the directory selection dialog (DIALOGTYPE=DIR).
(since iup-3.20)
...
=item B<MULTIVALUECOUNT>

I<(read-only)>

Number of returned values when MULTIPLEFILES=YES. It always includes
the path, so if only 1 file is selected its value is 2. (since iup-3.14)

=item B<MULTIVALUE>I<id>
...
without the path. (since iup-3.14)

=item B<MULTIVALUEPATH>

Force a full path in MULTIVALUE and VALUE attributes when
MULTIPLEFILES=YES (id=0 will still contain the path of the first file).
In Windows and Motif, only files in the same folder may be selected,
but in GTK when using the "Recent Files" files from different folders
can be selected. (since iup-3.20)
...
canvas. (since iup-3.20) B<NOTICE:> NOT working in Windows 7.
...
=item * L<0-basic/filedlg.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/filedlg.pl> - IUP::FileDlg example
