...
=item * B<USETITLESIZE:> Use the title size to define the cell
...
Action generated to retrieve the background color of a cell when it needs to be redrawn.
 ...
 sub bgcolor_cb_handler {
   ...
   #...
   return ($red, $green, $blue, $ret); #xxx check-the-implementation
 }
...
Action generated to retrieve the foreground color of a cell when it needs to be redrawn.
 ...
 sub fgcolor_cb_handler {
   ...
   my ($red, $green, $blue, $ret);
   #...
   return ($red, $green, $blue, $ret); #xxx check-implementation
 }
...
B<$drop:> reference to dropdown list element (IUP::List xxx) which will be shown to the user.
...
B<$drop:> reference to the dropdown list element (IUP::List xxx) shown to the user.
...
=item * L<RESIZE_CB|IUP::Manual::04_Callbacks/RESIZE_CB> and L<BUTTON_CB|IUP::Manual::04_Callbacks/BUTTON_CB> can be changed but you should
save and call the original definition from inside your own callback, or
the matrix will not correctly work. This can not be done in Lua (XXX).
