...
Action generated when multiple nodes are selected
...
Action generated when a drag & drop is executed.
...
Action generated when the right mouse button is
pressed over a node.
...
IUP::Tree has functions that allow associating a pointer (or a user
...
the userid will still be associated with the given node. In IUP::Lua,
instead of a pointer the same functions are defined for table and
userdata. These functions use the B<USERDATAid> attribute.

----

 int IUP::TreeSetUserId(Ihandle *ih, int id, void *userid); [in C]
...
additional feature of storing the Lua object in the registry.
 ...
 void* IUP::TreeGetUserId(Ihandle *ih, int id); [in C] 
...
object is retrieved from the REGISTRY.
 ...
 int IUP::TreeGetId(Ihandle *ih, void *userid); [in C] 
...
B<userid> to associate an userdata or table just like in B<iup.TreeSetUserId>. 
...
But the attribute string is complemented with the id value. For ex:
 ...
 IUP::SetAttributeId(ih, "KIND", 30, value) == IUP::SetAttribute(ih, "KIND30", value)
 IUP::SetAttributeId(ih, "ADDLEAF", 10, value) == IUP::SetAttribute(ih, "ADDLEAF10", value)
...
See also the L<IUP::TreeUtil|IUP::TreeUtil> contributed by
Sergio Maffra and Frederico Abraham. It is an utility wrapper in C++
for the IUP::Tree with some limitations.
