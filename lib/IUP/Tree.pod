=for comment based on iup-3.5 (attributes refreshed to iup-3.32) - http://www.tecgraf.puc-rio.br/iup/en/elem/iuptree.html
...
=item B<EXECUTEBRANCH_CB>

Action generated when a branch is executed. This action occurs when the
user double clicks a branch, or hits Enter on a branch.

B<Callback handler prototype:>

 sub executebranch_cb_handler {
   my ($self, $id) = @_;
   #...
 }
...
B<$self:> reference to the element (L<IUP::Tree|IUP::Tree>) that activated the event.

B<$id:> identifier of the branch node.
...
=item B<TOGGLEVALUE_CB>

Action generated when the toggle's state was changed. The callback also
receives the new toggle's state. (since iup-3.6)

B<Callback handler prototype:>

 sub togglevalue_cb_handler {
   my ($self, $id, $state) = @_;
   #...
 }
...
B<$self:> reference to the element (IUP::Tree) that activated the event

B<$id:> node identifier.

B<$state:> 1 if the toggle's state was shifted to ON; 0 if it was shifted to OFF.
...
was associated. C<SetUserId> must have been called for the node with
the given id.
...
if not found. C<SetUserId> must have been called with the same userdata_ref.
