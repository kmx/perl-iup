=for comment based on iup-3.5 (attributes refreshed to iup-3.32) - http://www.tecgraf.puc-rio.br/iup/en/ctrl/iupcolorbar.html
...
Use a 1 pixel flat border instead of the default 3 pixels
sunken border. When enabled is the same as setting SHADOWED=NO. Can be
Yes or No. Default: No. (since iup-3.24)

=item B<FLATCOLOR>

Color of the border when FLAT=Yes and the preview area
borders. Default "0 0 0". (since iup-3.24)

=item B<FOCUSSELECT>

When focus is changed the primary selection is also
changed. (since iup-3.29)
