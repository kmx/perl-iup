=for comment based on iup-3.5 (attributes refreshed to iup-3.32) - http://www.tecgraf.puc-rio.br/iup/en/elem/iupsbox.html
...
Controls the size of the bar handler. Default: 5. (since iup-3.27)
...
When the bar is moved automatically update the children layout. Default: YES. If set to NO then the layout will be updated only when the mouse drag is released. (since iup-3.27)

=item B<SHOWGRIP>

I<(non inheritable)>

Shows the bar grip affordance. Default: NO. When set to NO, COLOR is used to fill the grip area. If set to "LINES" then instead of the traditional grip appearance, it will be two parallel lines. (since iup-3.27)
