...
=head3 cdSaveState()
 ...
 $state = $canvas->cdSaveState(); #XXX-NOT-IMPLEMENTED-YET
...
=head3 cdRestoreState()
 ...
 $canvas->cdRestoreState($state); #XXX-NOT-IMPLEMENTED-YET
...
=head3 cdReleaseState()
 ...
 IUP:Canvas->cdReleaseState($state); #XXX-NOT-IMPLEMENTED-YET
...
B<ATENTION:> (XXX-CHECKTHIS) At the moment only the 
B<Cairo>, L<GDI+>, L<XRender> and L<IMAGERGB> drivers support alpha components in color
...
=head3 wdCanvasHardcopy()
 ...
 $canvas->wdCanvasHardcopy($ctx, $data, $draw_func); #XXX-NOT-IMPLEMENTED
...
=head3 cdGetImage()
 ...
 #NOTE: $img is a reference to IUP::Canvas::InternalServerImage object

XXX-TODO B<IUP::Canvas::InternalServerImage>
 
=head3 cdPutImage()

 #NOTE: $img has to be a reference to IUP::Canvas::InternalServerImage object
