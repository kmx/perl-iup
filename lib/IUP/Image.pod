...
=head1 DESCRIPTION, IUP::ImageRGB, IUP::ImageRGBA

Creates an image to be shown on a label, button, toggle, or as a
cursor.

(L<IUP::ImageRGB|IUP::ImageRGB> and L<IUP::ImageRGBA,|IUP::ImageRGBA,> since iup-3.0)

=head1 USAGE

=head2 CREATION - new() method

 $image = IUP::Image->new(int width, int height, const unsigned char *pixels);
 $imagergb = IUP::ImageRGB->new(int width, int height, const unsigned char *pixels);
 $imagergba = IUP::ImageRGBA->new(int width, int height, const unsigned char *pixels);
...
B<pixels>: Vector containing the value of each pixel. B< IUP::Image> uses
1 value per pixel, L<IUP::ImageRGB|IUP::ImageRGB> uses 3 values and B< IUP::ImageRGBA>
...
L<IUP::ImageRGB|IUP::ImageRGB> and L<IUP::ImageRGBA|IUP::ImageRGBA> in fact will be one value for each
...
is "1", the index "0" is ignored in IUP::Lua. Be careful when setting
...
"64 190 255"). If the value of a given index is
...
Used only for images created with L<IUP::Image|IUP::Image>.
...
=item * B<BPP >(read-only)

returns the number of bits per pixel in the image.
Images created with L<IUP::Image|IUP::Image> returns 8, with L<IUP::ImageRGB|IUP::ImageRGB> returns
24 and with L<IUP::ImageRGBA|IUP::ImageRGBA> returns 32. 

=item * B<CHANNELS >(read-only)

returns the number of channels in the image.
Images created with L<IUP::Image|IUP::Image> returns 1, with L<IUP::ImageRGB|IUP::ImageRGB> returns
3 and with L<IUP::ImageRGBA|IUP::ImageRGBA> returns 4. 

=item * B<HEIGHT >(read-only)
...
=item * B<RASTERSIZE >(read-only)

returns the image size in pixels. (since
3.0)

=item * B<WID >(read-only)

returns the internal pixels data pointer. (since
3.0)

=item * B<WIDTH >(read-only)
...
means of the L<IUP::Destroy|IUP::Destroy> function. To destroy an image, it cannot be
...
IUP::SetHandle|../func/iupsethandle.html>. You can also use
L<IUP::SetAttributeHandle|IUP::SetAttributeHandle> to shortcut the set of an image as an
attribute. For example:

 Ihandle* image = IUP::Image(width, height, pixels);
 
 IUP::SetHandle("MY_IMAGE_NAME", image);
 IUP::SetAttribute(label, "IMAGE", "MY_IMAGE_NAME");
 or
 IUP::SetAttributeHandle(label, "IMAGE", image); // an automatic name will be created internally
...
For example:
 ...
 IUP::SetAttribute(label, "IMAGE", "TECGRAF_BITMAP");  // available in the "etc/iup.rc" file
 or
 IUP::SetAttribute(label, "IMAGE", "gtk-open");  // available in the GTK Stock Items

In all drivers, a path to a file name can also be used as the attribute
value . But the available file formats supported are system
...
the X-Windows bitmap. For example:
 ...
 IUP::SetAttribute(label, "IMAGE", "../etc/tecgraf.bmp");
...
In Motif, the alpha channel in RGBA images is composed with the control
...
includes: B< IUP::Item> and L<IUP::Submenu|IUP::Submenu> always; and B< IUP::Toggle> when
...
IUP::ImageLib|../iupimglib.html>, a library of pre-defined images. To
view the images you can use the L<IUP::View|IUP::View> in the applications included
in the distribution, available at the L<Download|../download.html>. B<
IUP::View> is also capable of converting several image formats into an
IUP::Image, and save IUP images as LED, Lua or ICO.
...
for images in IUP::Lua.

=head2 IUP::Lua Old Constructor
...
=head2 IUP::Lua New Constructors
...
L<IUP::Toggle|IUP::Toggle>, L<IUP::Destroy|IUP::Destroy>.

The original doc: L<iupimage.html|http://www.tecgraf.puc-rio.br/iup/en/elem/iupimage.html>
