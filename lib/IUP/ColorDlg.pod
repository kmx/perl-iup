=for comment based on iup-3.5 (attributes refreshed to iup-3.32) - http://www.tecgraf.puc-rio.br/iup/en/dlg/iupcolordlg.html
...
=item B<COLORUPDATE_CB>

Action generated when the color is updated in the
dialog. It is also called when the color is updated programmatically.
(since iup-3.11)

The callback handler gets just C<($self)> - a reference to the
element (IUP::ColorDlg) that activated the event.
...
=item * L<0-basic/colordlg.pl|https://metacpan.org/source/KMX/IUP-0.305_1/examples/0-basic/colordlg.pl> - IUP::ColorDlg example (used for screenshot - IUP.pod)
