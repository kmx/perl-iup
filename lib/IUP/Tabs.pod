...
Image name to be used in the respective tab.
Use L<SetName|IUP::Manual::02_Elements/"SetName()"> or XXX-FIX_ME to
associate an image to a name. XXX-check-this

See also L<IUP::Image|IUP::Image>. In Motif, the image is shown only if
B<< TABTITLE(n) >> is C<undef>. In Windows and Motif set the BGCOLOR attribute
before setting the image. 

=item * B<TABORIENTATION> (non inheritable)
