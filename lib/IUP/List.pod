...
=head2 CREATION - new() method
 ...
 $list = IUP::List->new(const char *action);
...
=item * if "n" is set to NULL, all items after n are removed.
...
scrollbars are shown only if they are necessary. Default:
"YES".
...
If value is NULL removes all the items. Ignored if set before map.
...
=item * B<AUTOHIDE>=YES scrollbars are shown only if they are necessary, by
...
Depends on the DROPDOWN+EDITBOX
combination:
...
selected item. The value can be NULL for no item selected 
...
=item * B<VISIBLE_ITEMS >[Windows and Motif Only]
...
=item * B<APPEND, CARET, CARETPOS>, B<CLIPBOARD>, B<CUEBANNER,> B<FILTER,>
B<INSERT, PADDING>, B<MASK, NC, READONLY, SELECTEDTEXT, SELECTION,
SELECTIONPOS>, B<SCROLLTO>, B<SCROLLTOPOS> 

Same as the
=item * L<IUP::Text|IUP::Text> attributes, but are valid only when EDITBOX=YES
...
L<ACTIVE|IUP::Manual::03_Attributes/ACTIVE>, L<FONT|IUP::Manual::03_Attributes/FONT>,
...
B<$self:> reference to the element (IUP::XXXIUP_List) that activated the event

B<$text:> Text of the changed item.

=item * B<item> - Number of the changed item starting at 1.

=item * B<state> - Equal to 1 if the option was selected or to 0 if the option
was deselected.
...
=item * B<item> - Number of the selected item starting at 1.

=item * B<text> - Text of the selected item.
...
=item * B<state> - state of the list 1=shown, 0=hidden.
...
=item * B<value> - Similar to the VALUE attribute for a multiple selection list.
...
But the attribute string is complemented with the id value. For ex:
 ...
 IUP::SetAttributeId(ih, "", 3, value) == IUP::SetAttribute(ih, "3", value)
 IUP::SetAttributeId(ih, "INSERTITEM", 8, value) == IUP::SetAttribute(ih, "INSERTITEM8", value)
...
L<IUP::ListDialog|IUP::ListDialog>, L< IUP::text|iuptext.html>
