=for comment based on iup-3.5 (attributes refreshed to iup-3.32) - http://www.tecgraf.puc-rio.br/iup/en/elem/iuplist.html
...
Prepare the Drag & Drop callbacks to support drag and drop of items
between lists (L<IUP::List|IUP::List> or L<IUP::FlatList|IUP::FlatList>),
in the same IUP application. Drag & Drop attributes still need to be set
in order to activate the drag & drop support, so the application can
control if this list will be source and/or target. Default: NO. (since
iup-3.10)
...
I<(non inheritable) (write-only) [Windows and GTK Only]>

Image name to be used in the specified item, where n is the specified
item starting at 1. The item must already exist. Use
L<IUP::SetHandle|IUP::SetHandle> or
L<IUP::SetAttributeHandle|IUP::SetAttributeHandle> to associate an image
to a name. See also L<IUP::Image|IUP::Image>. The image is always
displayed at the left of the text and only when SHOWIMAGE=Yes. When
EDITBOX=Yes the image is not displayed at the edit box. Images don't
need to have the same size. In Windows, list items are limited to 255
pixels height. (since iup-3.6)
...
Enables the internal drag and drop of items in the same list, and
enables the B<DRAGDROP_CB> callback. Default: "NO". Works only if
DROPDOWN=NO and MULTIPLE=NO. Drag & Drop attributes are NOT used.
(since iup-3.7)
...
I<(creation only) [Windows and GTK Only]>

Enables the use of an image for each item. Can be "YES" or "NO".
Ignored if set after map. (since iup-3.6)
...
Same as SPACING but using the units of the vertical part of the SIZE
...
Changes or retrieves the value attribute using a string of an item.
Works only when EDITBOX=NO and DROPDOWN=YES, or DROPDOWN=NO and
MULTIPLE=NO. When set it will search for the first item with the same
string. (since iup-3.12)

=item B<VALUEMASKED>

I<(non inheritable) (write-only)>

Sets VALUE but first checks if it is validated by MASK. If not does
nothing. Works only when EDITBOX=YES. (since iup-3.13)
...
Number of items that are visible when DROPDOWN=YES is used for the
dropdown list. Default: 5.
...
active if B<SHOWDRAGDROP=YES>. (since iup-3.7)
...
B<$isshift:> flag indicating the shift key state.

B<$iscontrol:> flag indicating the control key state.

B<Returns:> if returns IUP_CONTINUE, or if the callback is not defined
and SHOWDRAGDROP=YES, then the item is moved to the new position. If
