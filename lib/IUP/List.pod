...
=head2 CREATION - new() method
 ...
 Ihandle* IUP::List(const char *action); [in C]iup.list{} -> (elem: ihandle) [in Lua]list(action) [in LED]
...
=item * B<text> - Text of the changed item.
...
Use xxxTODO??? L<IUP::ConvertXYToPos|IUP::ConvertXYToPos> to convert (x,y)
...
(ignored at the editbox). Use L<IUP::ConvertXYToPos|IUP::ConvertXYToPos> to convert (x,y)
coordinates in item position. 
