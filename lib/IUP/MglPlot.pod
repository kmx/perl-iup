=for comment based on iup-3.9 (attributes refreshed to iup-3.32) - http://www.tecgraf.puc-rio.br/iup/en/ctrl/iup_mglplot.html
...
Examples of some of the supported plot modes:
...
L<PlotLoadData|/"PlotLoadData()"> or C<SetFromFormula()> functions only.
...
Suppress the print of warning messages in stderr. Can be Yes or No. Default: No. (since iup-3.21)
...
Internal MathGL margin configuration of the dataset area. Can be Yes or No. Default: "Yes". (since iup-3.11)

=item B<MGLGRAPH>

I<(read-only)(non inheritable)>

Returns the B<mglGraph>* handle used internally. (since iup-3.11)

=item B<OPENGL>

I<(non inheritable)>

Enable or disable the rendering in OpenGL.
